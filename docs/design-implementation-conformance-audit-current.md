...
| Traveller meetings/voice | **Source-complete; play review pending** | The retained 8-meeting catalogue baseline plus the 23-entry generated authored corpus resolve all 29 travellers; Noll and Auber are intentional replacements, and every resolved meeting has three owner-scoped independent replies | Preserve the generated-corpus overlay as the sole promotion path; review in campaign order during play and revise exact copy from reports rather than waiting on advance Atlas approval |
...
| Creatures/bestiary | Source-complete ecology; partial visual consumption | Generated species/specimens, derived combat/loot/names and truthful Bestiary records. `4a7b73e` adds persisted visual-only `bodyPlan` + `cranialFeature` with independent appendages and no gameplay-RNG change; Encounter consumes the native procedural identity. Accepted AssetLab schema-v5 evidence proves distinct fennec-like and winged-serpent placeholders (`92271d14…a20e`). | Bestiary tiles/specimen cards and the explorable world map still need the same species identity consumer; the minimap deliberately remains a generic disclosure marker. Final creature art and longer-campaign balance remain outstanding. |
