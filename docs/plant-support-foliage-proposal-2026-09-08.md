# Plant support and foliage proportions — proposal for Aimee review

8 September 2026. **PROPOSAL ONLY. Not an accepted rule, implementation assignment or change to installed354.** PM requested one concrete response to the retained fibrous/fleshy readability findings. The recommendation below changes visual construction only if Aimee approves it. Ratios are Design-authored proposed tuning, not values measured from screenshots or already approved by Aimee.
...
| Existing form | Proposed mapping | Purpose |
| --- | --- | --- |
...
If approved, Engineering supplies a separately frozen flora-only morphology path for newly bound books; do not reinterpret saved form2/component5 recipes or change creature forms. Component meshes and surface2 remain independently versioned. Preview, actual preparation, memory and reopen must share the selected path. Existing books and unfinished saved expeditions keep their exact old geometry.

After approval, validate this as one coherent revision using the retained opposed/basal blade and fleshy needle/frond sources, plus focused existing source/contact/persistence checks. Preserve a same-source comparison at the actual target/default/current appearance; do not run a world census, parameter sweep, configuration matrix or repeated component microbatch. The intended acceptance is a recognizable foliage-bearing upright plant, a recognizable basal rosette, and fleshy branch fans emerging outside the body with seated visible joins. Natural rear occlusion is allowed. Correct counts alone do not pass visual readability, and an image alone does not pass hidden contact. The already-readable branched woody/fibrous recipe fields should remain unchanged.

## Decision to route through PM

**Recommended:** approve this one arrangement-aware proposal for a bounded implementation and same-source visual comparison. Tradeoff: compact basal silhouettes and wider exposed fleshy branch fans, with old books unchanged. Approving a prototype would not certify its final appearance; materially different ratios or wider family scope would need an explicit revision to the proposal. Until Aimee accepts, this document and the Wiki proposal create no implementation authority. PM owns presenting the decision and collecting her response.
