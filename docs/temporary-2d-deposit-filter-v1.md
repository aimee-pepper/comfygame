...
**Consumer implemented in342; artwork complete internally, integration pending:** Engineering `docs/resource-node-renderer-seam-2026-09-07.md` in world-crash-and-notifications names `Sources/VisualRuntime/OrthographicResourceNodeRenderer.swift`, called by `OrthographicWorldPrototypeView` using sanitized `OrthographicResourceNodePresentation` from the adapter. Input is disclosed nondepleted source identity, producer/material, completedHits/requiredHits; depleted sources yield no presentation. One tile=one unit, +Y up, ground-contact tile-center root; X/Z±0.45,Y0…1. Output Swift/RealityKit or bundledUSDZ, optionalPNG≤512×512;≤32meshparts/4096triangles per node. Grey boxes remain unfinished artwork. This is the ready consumer for the already-assigned Asset3D node batch, not a new2D export or additional trial.
