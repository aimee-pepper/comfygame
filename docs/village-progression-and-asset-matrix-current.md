...
| Library | Wider Library corner appears in the Binder House. Its destination is a close-up of one large bookcase, at most two adjoining cases only if target geometry requires it. Five clickable shelf sections: Diaries, Bestiary, Dictionary, Field Notes, World History. One empty shell plus reusable paper/folio/book/volume sprites are deterministically composed from real records; empty/early/developed are fixtures, not baked state art. Diaries add one book per recovered traveller and grow that book from its own page count; Bestiary and Dictionary are multi-volume sets. |
