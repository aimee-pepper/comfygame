...
**Decided second layer, approved by Aimee on 5 September:** body shape. Existing values give Four-legged, Two-legged, Serpentine, Segmented, Radial, Fish-shaped and Amorphous. For example, Sky → Serpentine → species, or Water → Fish-shaped → species. Use the axial body plan rather than counting all wing/fin/accessory appendages. New generation still follows the permitted body/habitat combinations; sorting preserves existing recorded facts even where a newer generator would not choose them. The knowledge-and-grouping contract now selects nonempty recorded body-shape content groups, without a completion target for every theoretical combination. Fixed chapter navigation labels remain permitted.
...
The approved sorting UI now has an exact [knowledge and grouping contract](bestiary-knowledge-and-grouping-v1.md): recorded-only admission, explicit body-shape provenance, legacy fallback and nonempty content groups. It does not add remote-sighting discovery or material revelation.
