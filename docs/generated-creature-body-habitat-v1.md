...
5. Preserve the original sampled count where legal (the separate future cold-water opt-in in [its contract](creature-cold-water-appendages-v1.md) narrowly supersedes that count rule); none becomes zero, other types clamp to 1–8, aerial wings and shore-piscine limbs clamp to 2–8. Keep the existing cranial choice. No count reroll, new budget spend or costly-axis edit.
...
**7 September Aimee clarification:** dark-water starts beyond player reach are valid for supported aquatic types. They may approach shallow water only after actual detection of the player. Current contact at the spawn tile is not required, and an allowed remote-only component must not be rejected by a universal contact-route validator. Preserve existing reserved contact-component slots separately; no forced approach, hidden-actor reveal or new swimming/encounter reach. The later terrain scenery mask still excludes its specifically designated high scenery.
