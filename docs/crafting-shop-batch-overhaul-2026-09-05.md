...
**Weaponsmith production closed:** `weaponsmith-whole-shop-production-v1.md` supplies all four families/six physical kind rows, actual Haft/collar production, retained diary Polearm access, Balanced/Driving choices using existing stats, complete values/recovery/refit and legacy migration. Its new calculator replaces automatic old+.5 offsets and six-band fees/caps for future crafts. No individual fitting question or unnamed material remains; existing Forge/Bowyer baseline comparisons are included. Armoury is the next whole batch.
...
The complete production replacements now exist: `distillery-whole-system-production-v1.md` and `channelworks-whole-system-production-v1.md`. They define named Resin/Toxic Sap/Silver routes, all three new potency60 Cores, all nine housing combinations, complete output arithmetic, foundation/restoration, source colours, retune/rebuild/recovery and old-save preservation. They supersede this batch's earlier open ingredient and numeric placeholders. No Blank Core, one-strike coating comparison or unsupported Light illumination may re-enter through legacy compatibility. The new systems remain pending until Engineering reports their actual equipped consumers delivered.
