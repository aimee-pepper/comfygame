...
- The second-tap default is described in permanent action-bar copy, not another opening card:
  “Choose a target—or press Attack again to use the shown default.” The UI must name whether that
  default is the selected skill, the Binder's gambit result, or the first standing foe.
...
| 2 | `people.firepit_party.v1` | Firepit opens after the first recruit | First active-party membership change, or explicit confirmation that the current travel party is kept | Firepit travel controls: **The Firepit chooses who travels.** People left at the Base are not lost; the party limit counts the Binder plus the companions marked as coming. |
...
Declining is not recruitment and must not set `first_traveller_recruited`. A full Firepit disables the
invite with the existing concrete reason before the decision; the tutorial cannot imply the person
was seated. If a decline is followed by collapse or tile loss, ordinary world rules decide whether
the opportunity survives—no tutorial protection or respawn is added.
...
8. Attempt recruitment at capacity; no found/recruited fact is written and the person can recur.
