...
## Other resource words and gems — current explanation and separate proposal

Rich Ore and Gilded Veins are different existing Compounds: the first writes strong Iron and ordinary Gold; the second writes strong Gold and faint Crystal. Their actual pressure effects can influence suitable mineral and stone opportunities, so this is not a metals-only rule. Crystal-derived sourceless light can also satisfy the planned natural Essence-node preference. Old flat reward multipliers in historical data are not current rewards and are not being restored. Further Essence-yield bonuses or gemstone secondary drops remain proposals. A personal Compound follows its actual marks rather than a suggestive title. The current material catalogue includes useful Quartz, but a broader gem family and jewelry-production system are not delivered. A small set of useful secondary gem finds from appropriate hosts is under discussion; it is not a new guaranteed reward or an approved catalogue.
