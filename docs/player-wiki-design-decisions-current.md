...
Quartz retains its existing functional recipes. Garnet and Beryl start at the existing uncommon barter value band. All three are ungraded exact-name resources: colour and source history stay inside the stack and the chosen colour carries into its inlay. Ordinary refitting preserves the inlay; explicit replacement/removal and recycling return its original gem once. An inlay adds no automatic stat or resale multiplier and emits no light. Existing worlds and stock keep their original rewards. New drops will arrive together with their first usable fitting and finished inventory icons, not as unused collection clutter.
...
**Current behavior — installed in382:** Build382 restores matching static textures for already-known ground in older explored areas without appearance records, and for permitted partially visible terrain. Remembered plants retain their saved colours instead of appearing grey. Build382 introduced a broad pending-movement ripple, now awaiting the slow-terrain-only correction below; currently visible, nondepleted collectibles have a restrained local glint. A modest neutral edge improves current creature contrast while retaining each source colour. Existing saved-action gathering feedback still follows your Mining and gathering results preference.

**Current movement response:** The installed build uses a broader pending-movement ripple introduced in382. Aimee reports that walking appears to wait for it in every direction; the cause of that delay is still being investigated. The correction below is decided but is not yet verified as delivered.
