...
**Reusable flora artwork — current in349, newly bound books in the existing3D view:** broad woody leaves, branches and trunks now use the reviewed Blender artwork, assembled using each plant’s existing size, arrangement, source colour and saved structure. This adds no new species type, organ, harvest or growth rule. Newly bound books use these reviewed components; existing books and saved plants keep their recorded appearance, including books created with the earlier leaf/branch pair. The normal2D view is unchanged. Trunks retain the existing plant structure, with branches and thorns seated on the visible surface. This changes appearance only. The next fibrous blade artwork has passed its internal component review but is not included in349. **Unresolved appearance issue:** at ordinary map scale, the stem still dominates these plants and the narrow leaves are difficult to distinguish, especially near the base. A coordinated review of plant proportions is still needed; no replacement proportions have been decided, and the component review does not mean the whole plant’s appearance is finished.
