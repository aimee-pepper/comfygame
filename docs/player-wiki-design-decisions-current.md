...
**Notification progress — field copy delivered in346:** routine field events, Schematic notices and auto-path warnings now use shorter results or actual refusal reasons while preserving the underlying event and prose. Survey notices retain concise name-and-value readings because the Measured view is not always unlocked. Crumbling reports uncollected finds rather than calling every result an item. The event-history sizing and position are also corrected. Remaining action, shop, refusal and combat wording is still being updated; necessary targets, quantities, requirements and durations remain part of the accepted rule.
