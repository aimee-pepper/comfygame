...
## Movement responsiveness — current regression open

**Current behavior:** Build403 corrects the reproduced movement lock caused when a collapsing tile destroyed an uncollected teaching. The existing saved campaign can continue with its next legal step, without relocation or a reset. Checks on a copy of the affected campaign covered all four directions, an actual movement input and reopening the saved result; stock and recovered teachings remained unchanged. Installation and ordinary iPhone launch succeeded, and Aimee confirms she can move again. Severe movement lag remains open. Build404 keeps an already displayed notification visible while the next scene is prepared, preserving its text and original expiry until a valid replacement arrives. Internal recordings showed no blank frame during the inspected replacements. Internal processing measurements also improved, but do not establish prompt visible movement. Installation is verified; ordinary launch was unavailable because the phone was locked, so physical-phone appearance and usability are unverified for404. Responsive movement and a matching visible lit area still need correction. Earlier internal checks showed some matching movement and lighting, but did not establish the responsive experience required. Correcting this must preserve prompt movement and a matching lit area together.

**Decided intended behavior:** Movement must respond immediately while the character and its visible lit area move together. Delaying the character until the light catches up is not an acceptable fix. North-pointing idle ripples must appear behind the character where they overlap; this layering correction is pending. This applies with or without a Torch and whether the view moves or stays fixed, while keeping the existing discovery, saving and turn-cost rules.

## Gameplay notices — corrected in403

**Current behavior:** Build403 removes internal world-repair notices and acknowledgements from gameplay, including old pending notices after reopening. It also restores the actual source’s refusal explanation when you attempt gathering underfoot, without spending a turn or switching to an adjacent source. Internal checks covered a real refusal attempt with optional gathering messages muted, preserved saving and repair failure handling, and reopening. Installation and ordinary launch succeeded; physical-phone gameplay was not inspected.

**Decided intended behavior:** World maintenance and saved-world repairs must not interrupt play with a notice or acknowledgement. Normal gameplay feedback stays: when you try to gather from a source you cannot work, explain that source’s actual unmet requirement or refusal. Refusal spends no turns or materials and does not switch to a different source. Your notification preferences stay unchanged.

## Current plants, creatures and nodes — visual quality open

Currently visible plants, creatures and resource nodes are reported to look flat and desaturated beside the textured terrain. This remains an open visual-quality problem; earlier checks of individual sites do not establish that these materials are finished. The intended result is coherent, richly detailed artwork that preserves each source’s actual colours and materials. Dim remembered appearances are a separate sight treatment, not the explanation for objects currently in full view.
...
## Current Explore corrections — 13 September

**Current:** Build399 shows directional ripples around your stationary character before movement input, pointing toward currently visible, reachable neighbouring terrain that takes extra turns to cross. Several slow directions can appear together; ordinary-cost and blocked directions produce no slowing ripple. The warning clears when movement starts and returns for your known surroundings after you stop, without adding a pause or changing costs. The character and visible bright area now share a movement check, including without a Torch and where the view stays fixed. The installed approach can hold movement until a matching saved view is ready; this delay is a reported regression and is not an accepted solution. Internal review of a naturally played campaign copy, reopened before input, verified east/south/west warnings, no false north warning, and their clearing and return across two movement boundaries. Installation and ordinary iPhone launch succeeded. Noticeable frame holds remain; this is bounded verification, not physical-phone gameplay acceptance, universal smoothness or a complete lighting certification. Build399 automatically uses an applicable tool already available to the gathering action, without changing your saved preference or swapping equipment. A valid chosen source takes priority; otherwise a live source under your character comes first across resource types. With nothing underfoot, a workable adjacent source comes before one whose requirement you cannot meet. The selected name, direction, action, base highlight and availability now agree in the compact and expanded context. Tool requirements remain readable, with the full explanation available by expanding the panel. Existing tool eligibility, reach, costs and saved harvest rules remain.

**Current Workshop artwork — installed in401:** Build401 adds the recognizable Binder’s Workshop: a stone writing room with a walnut bench, paper and brass details. The central standing space stays clear, and a completed search changes its drawer while the room remains. Internal checks and art review covered unsearched, searched, occupied and remembered appearances. Workshop installation is verified, and ordinary launch subsequently succeeded in402. Physical-phone appearance and interactive usability remain unverified. Search effort, rewards, movement and saved-world generation keep their existing rules.

**Decided intended behavior:** While your character is standing still, directional ripples should point toward every currently visible, reachable adjacent tile that takes extra turns to cross, before you press a movement control. No tap or pending action is required. Ordinary-cost and inaccessible directions have no slowing ripple, and hidden or merely remembered terrain reveals no new information. The ripples disappear when visible movement starts and return for the known surroundings after you stop. They add no delay, turn cost or saving-progress indicator. The character and its currently visible bright area must stay aligned on every movement path, with or without a Torch and whether the view pans or stays fixed. Movement must preserve the existing sight boundary: no new surroundings appear before their saved result. Movement must respond immediately while the character and its visible lit area move together. Delaying the character until the light catches up is not an acceptable fix. North-pointing idle ripples must appear behind the character where they overlap; this layering correction is pending. This applies with or without a Torch and whether the view moves or stays fixed, while keeping the existing discovery, saving and turn-cost rules. The automatic-tool and source-selection rules are delivered in399. Gathering keeps each source’s existing tool and access requirements; choosing a source does not equip gear, buy anything, spend a turn or activate a site or portal. Existing access limits remain: some older resource nodes prevent gathering certain neighbouring sources while you stand on them. Step to a legal working position beside the source to gather it.

**Current target selection —399:** Build399 replaces the Interact tool chooser with source selection when several gathering sources are in reach. Hold for 0.40 seconds, keep holding while sliding to a source, then release to select it. Release outside to cancel. The next ordinary tap performs the labelled action; selection itself never harvests or spends a turn. A chosen source whose requirement you cannot meet stays selected and explains that requirement. Cancelling or releasing a stale choice does not perform another action. The choice clears when you move, leave or lose access to that source. The explicit Field Kit remains available.

These corrections are delivered in399 and require no repeated decision or homework from Aimee. Broader smoothness remains unfinished;400 delivers water flowing into chasms for newly prepared books.

## Slow-terrain feedback and coherent movement

**Current behavior and reports:** Build399 shows directional ripples around your stationary character before movement input, pointing toward currently visible, reachable neighbouring terrain that takes extra turns to cross. Several slow directions can appear together; ordinary-cost and blocked directions produce no slowing ripple. The warning clears when movement starts and returns for your known surroundings after you stop, without adding a pause or changing costs. The character and visible bright area now share a movement check, including without a Torch and where the view stays fixed. The installed approach can hold movement until a matching saved view is ready; this delay is a reported regression and is not an accepted solution. Internal review of a naturally played campaign copy, reopened before input, verified east/south/west warnings, no false north warning, and their clearing and return across two movement boundaries. Installation and ordinary iPhone launch succeeded. Noticeable frame holds remain; this is bounded verification, not physical-phone gameplay acceptance, universal smoothness or a complete lighting certification. In the reviewed movement sequences using normal controls, the party stayed visible, a known slow-terrain cue appeared while stationary and was absent by arrival, and destination details and the crossing notice followed arrival. Newly entered tiles no longer appeared black in the three inspected crossings. These historical samples do not close the current idle-warning or general lit-field defects. Installation and ordinary iPhone launch succeeded. A separate phone material check confirmed the lighting response, without interacting with campaigns; appearance and movement in a normal campaign on the phone remain uninspected. The rule still requires the ripple to clear at the first visible movement without adding a pause. The character and its currently visible bright area must stay aligned on every movement path, with or without a Torch and whether the view pans or stays fixed. Movement must preserve the existing sight boundary: no new surroundings appear before their saved result. Movement must respond immediately while the character and its visible lit area move together. Delaying the character until the light catches up is not an acceptable fix. North-pointing idle ripples must appear behind the character where they overlap; this layering correction is pending. This applies with or without a Torch and whether the view moves or stays fixed, while keeping the existing discovery, saving and turn-cost rules. Build399 extends that check to the reported no-Torch and fixed-view paths; broader responsiveness remains unfinished.

**Decided intended behavior:** While your character is standing still, directional ripples should point toward every currently visible, reachable adjacent tile that takes extra turns to cross, before you press a movement control. No tap or pending action is required. Ordinary-cost and inaccessible directions have no slowing ripple, and hidden or merely remembered terrain reveals no new information. The ripples disappear when visible movement starts and return for the known surroundings after you stop. They add no delay, turn cost or saving-progress indicator. A completed slow crossing reports its actual total and extra turns, following your existing notification settings. The character, view and lighting should stay aligned without revealing undiscovered surroundings or causing an extra move.

## Raised ground at the map edge

**Current behavior — installed in392:** Build392 restores side faces at the actual world boundary: known solid ground above elevation zero shows its own side down to zero. This creates no ground outside the world and does not reveal unknown neighbouring tiles. Internal checks and art review accepted the restored elevation1 edge in the unchanged view. Installation and ordinary iPhone launch succeeded; physical appearance and interactive usability have not been independently inspected.
...
**Decided behavior:** Walking onto a place must never hide or remove it. Sites stay in the world with their committed unlooted or looted appearance; entering or starting a search does not complete it. No intermediate looted appearance is being added now. A canopy or roof may soften only where it blocks the character, while the actual place stays visible. Portal stone is fully opaque; only the blue magic may be semitranslucent. Build394 Torch light is blocked by the portal’s actual stone frame while its blue veil and moving magical ribbons let light pass. This changes no portal artwork or travel rule. Installation is verified; ordinary launch subsequently succeeded in395 after the earlier394 locked-phone attempt. Physical appearance and interactive usability remain unverified. A loose object can leave the ground when it is actually collected, including an existing automatic pickup during a successful step; simply standing over an uncollected object must not hide it.

Fringe must show vague permitted terrain, and explored areas must retain discernible dim, still memories of terrain and known non-enemy objects. Only truly unknown space is wholly undisclosed. Restore those views without expanding sight, revealing hidden enemies or refreshing hidden changes. The outward fade belongs to never-explored fringe, not remembered terrain. The bounded visibility correction is installed in391. Movement and light timing corrections remain unfinished; stacking corrections follow later.
...
## Portals, camp, Workshop and Vault — installed; remaining site artwork — intended

**Current behavior:** Build390 delivers authored stone portals with flowing blue Essence magic and a recognizable Wayfarer’s Camp. Its chest changes from closed to open after a committed search, while the shelter, bedroll and cold hearth remain. Both portal identities keep their actual Return home behavior. Beyond the camp, Workshop and Vault, site families still use unfinished markers. Installation and ordinary launch were verified for these earlier portal and camp additions; the earlier build401 launch limitation was subsequently cleared by the successful ordinary launch of402.

**Decided intended behavior:** The remaining exhausted housings, caverns, geyser basins, warrens, Tears and Atlas Seams will receive recognizable artwork of their own. Searching changes a place to searched or emptied where appropriate without erasing its identity or inventing physical changes. Spent Emanation Housing is already exhausted before search, and Atlas Seam keeps its separate anchoring purpose.

**Current Vault artwork — installed in402:** Build402 adds the Glacial Vault’s recognizable frozen enclosure, set behind a clear standing space. Completing its search opens the door inward while the structure remains. The ice body blocks sight and Torch light even where its artwork looks partly translucent; real openings let sight and light through. Remembered appearances remain dim and last observed, without hidden updates. Internal checks and art review covered the corrected unsearched, searched, occupied and remembered appearances. Installation and ordinary iPhone launch succeeded; physical-phone appearance and gameplay have not been inspected. Search, rewards, movement and discovery rules keep their meanings. This artwork update does not fix the open movement-response regression.
...
Build387 changed interaction labels only; its installation and ordinary launch were verified. Build388 adds the bounded camera-follow and Apex presentation corrections described here. Build389 adds the shared information-pane placement. Build390 adds authored portals, Wayfarer’s Camp and solid-side contrast. Build391 corrects occupied-site visibility, portal opacity and fringe/remembered-terrain readability. Build401 adds the Workshop artwork described above. Remaining site artwork, the reported grey appearance of plants and creatures, and moving lights remain unfinished.

## Movement response — installed in388

Build388 makes the character and view respond together on the existing safe-step path to an already visible neighbouring tile. Selection, alert and work markers stay aligned with that scene. New details, saved world position and the next action still wait for the saved result; a failed save restores the character and view together. Steps involving encounters or other events can still take longer. This is a bounded camera-follow correction, not instant movement everywhere or a completed fix for every visual effect. Installation is verified. The earlier launch attempt was blocked by a locked phone; ordinary launch subsequently succeeded in390. Physical-phone appearance and interactive usability remain unverified. The misleading pending/completion ripple remains removed. Terrain and action costs keep their existing rules. Build396 checks covered a cue after input. Build399 delivers the corrected idle warning before input, with bounded campaign-copy verification. A completed crossing reports its actual total and extra turns; for example, crossing mud can take 4 turns, 3 extra. Ordinary-cost directions must have no slowing ripple; known adjacent slow directions should be indicated while idle, before input. Build397 reduces the delay in matched internal recordings, but movement responsiveness remains unfinished; physical-phone touch-to-visible-movement timing was not measured.

## Apex, solid sides and chasm walls — installed; chasm waterfalls — intended

Build388 replaces the main-view Apex label with a static threatening red aura around the rendered creature, keeping its body visible through the open centre. The minimap uses a red dot contained within its tile. The aura appears only for an Apex in current full sight; it does not appear in fringe sight or memory, illuminate the world, reveal hidden creatures or change combat. Internal checks and art review cover the displayed aura and its alignment during a map pan. Installation is verified, and ordinary phone launch subsequently succeeded in390. Physical-phone appearance and interactive usability remain unverified. This does not restore anatomy missing from an older saved creature.

Build390 darkens textured solid terrain sides to distinguish them from their tops, preserving their source hues and textures. Top surfaces, terrain geometry and existing sight dimming keep their rules. This is a side-contrast correction, not a new lighting system. Build392 shows the adjoining ground’s side descending about two visible levels into darkness when the ground and chasm edge have been observed together. This creates no physical bottom or walkable floor. Unknown edges stay hidden; remembered edges use their recorded height and material without hidden updates. Older memories with no recorded edge do not gain guessed detail. Internal checks and art review accepted the current wall depth and downward fade. Remembered outlines remain discernible, but the fine wall gradient is too dark for detailed acceptance. Installation and ordinary iPhone launch succeeded; physical appearance and interactive usability have not been independently inspected. Build400 allows an actual flowing channel in a newly prepared book to end at its recorded open lip and fall into darkness, without a receiving pool. Still water, ice and an unselected or blocked edge do not automatically spill. This applies to newly prepared worlds under a saved policy, preserves water amounts, and requires the source and chasm boundary in full sight for live flow; memory stays still and last observed. Chasm walls are delivered in392. Generating these chasm outlets and showing their current and remembered waterfalls are delivered in400 for newly prepared books. Internal checks covered genuine one-cell and longer outlets, water and freezing records, safe saving, unchanged older-book rules and current versus remembered appearance. Native art review confirmed a visible falling curtain at the source lip and a dim, static remembered curtain fading into black without a bright bottom cap. Build400 installation and ordinary iPhone launch are verified. This does not establish a physical-phone gameplay walkthrough, smooth animation on every frame, a guaranteed scenic waterfall or completion of the separate world-generation overhaul. Build400 retains the earlier overhead and Torch lighting and399 movement corrections; broader responsiveness and lighting polish remain unfinished. Aimee likes the appearance of the holes left by fallen tiles; this does not establish acceptance of every waterfall or all chasm effects.
...
**Current behavior:** Build389 makes notifications and At this place switch between top and bottom together, using the existing two-tile opposite-edge rule. Build404 corrects the inspected notification flicker during scene preparation: the previous message remains until the next message can appear, unless its original expiry or dismissal ends it first. This does not change panel placement or extend a message’s lifetime. Movement controls, minimap, Use Tile, Look, carried items and Field Kit stay fixed at the bottom. The map keeps its size, scale and position. Open pane details, the current location, exact Read now links and notification expiry stay intact during the switch. If neither edge is suitable, both panes stay at their existing shared edge rather than jumping back and forth.

Details open inward, and notification arrival or expiry alone does not move At this place. Internal checks cover both edges, exact page links, expansion, notification lifetime and safe touch handling. Installation is verified. The earlier launch attempt was blocked by a locked phone; ordinary launch subsequently succeeded in390. Physical appearance and interactive usability remain unverified.
...
## Overhead light — installed in393; Torch light — installed in394; other sources — intended

**Current behavior:** The campaign now has supported overhead lighting tied to its world clock, with its existing Torch sight rules. Build393 delivers supported overhead lighting; Torch surface illumination is installed in394, while luminous resource-node lights remain planned.

Build393 delivers supported Sun and Moon lighting and shadows from the world’s actual committed phase and light sources. Already-visible surfaces remain readable in dark worlds through neutral shading that does not increase sight or brighten memory. Internal checks and art review cover real Sun movement, dark worlds with and without an active Torch, and corrected sampled shadows. Installation and ordinary iPhone launch succeeded; physical appearance, temporal smoothness and shadow contact on every shape have not been independently verified. Overhead motion follows committed world turns and the world’s existing day/night cycle; waiting or leaving the game does not move the sun. Planned local lighting will allow explicitly luminous nodes or veins to cast light; ordinary resources will not automatically glow. The planned Essence node is an appropriate blue local emitter; the currently delivered loose crystal keeps its existing aura. Rendered light does not itself grant sight or change source colours stored for crafting. Ground and objects already in full sight must stay readable even in dark worlds, including terrain revealed by an active Torch. That readability does not add a sun, reveal unknown ground or brighten remembered areas. Overhead lighting and the Torch’s visible local glow can arrive separately only while existing visibility is preserved; overhead lighting is delivered in393 and the activated Torch’s visible local light is installed in394.

Build394 shows an activated Torch in the party’s hand and casts warm light up to three tile lengths onto ground and objects already in full sight. Build396 matched the character and view by delaying some steps, and removed black entering cells in inspected crossings. Delaying movement this way is now explicitly rejected as a solution; the responsiveness regression remains open. Build399 shows directional ripples around your stationary character before movement input, pointing toward currently visible, reachable neighbouring terrain that takes extra turns to cross. Several slow directions can appear together; ordinary-cost and blocked directions produce no slowing ripple. The warning clears when movement starts and returns for your known surroundings after you stop, without adding a pause or changing costs. The character and visible bright area now share a movement check, including without a Torch and where the view stays fixed. The installed approach can hold movement until a matching saved view is ready; this delay is a reported regression and is not an accepted solution. Internal review of a naturally played campaign copy, reopened before input, verified east/south/west warnings, no false north warning, and their clearing and return across two movement boundaries. Installation and ordinary iPhone launch succeeded. Noticeable frame holds remain; this is bounded verification, not physical-phone gameplay acceptance, universal smoothness or a complete lighting certification. The flame and warm light appear at night or when ambient light is below25, fading smoothly between20 and25 in daytime; the Torch body and its trip-long sight effect remain in daylight. The check uses ambient light before the Torch’s bonus. Merely carrying the item does not activate it, and existing consumption and turn costs remain unchanged. Light falls away with distance and is blocked by supported physical terrain, trunks and structures while preserving their real openings. Hidden enemies cast no revealing shadows, and remembered, fringe and unknown areas receive no new Torch illumination or discovery. Committed strength changes use the existing short250ms transition. Internal checks and art review cover the carried model, light pool, supported materials, activation, failed saves and restored state; they do not establish every material finish or smoothness on every shape. Installation is verified. The394 launch attempt was blocked by a locked phone; ordinary launch now succeeds in397. Physical appearance and interactive usability have not been independently verified. Lantern and Light Inscription remain separate intended work and must complete activation before emitting light.

Distant lights are now decided intended behavior: if an active luminous object is in unobstructed line of sight, its vague outline and the ground inside its actual light radius can appear at fringe detail. Terrain, opaque sites and tree trunks can block the source or parts of that light. Build395 makes ordinary sight, discovery and observations use the same supported physical terrain, structure and growth shapes as local light. The near face of an obstruction can be seen while the ground beyond stays blocked; real portal openings remain clear. Sight ranges, party and Torch effects, enemy visibility rules, movement and costs stay intact. Previously explored places remain remembered when their current view becomes blocked. Newly bound worlds begin with discoveries based on this same view; existing worlds are not regenerated. Internal checks cover sight and discovery agreement, normal entry, failed saves and reopening. Installation and ordinary iPhone launch succeeded. Physical appearance, interactive playthrough and smoothness on every shape have not been independently verified. Distant-light perception still needs a supported stationary source and its in-world presentation in normal expeditions; the loose Essence crystal’s decorative aura does not qualify. This does not expose nearby creatures or resource contents, reveal an unlit route, mark the patch explored or allow remote actions. Approach under normal sight rules for full detail and discovery. The effect disappears when you lose sight of the light; remembered objects keep their last-observed state underneath. Sources outside this permitted perception and hidden creatures still cast no revealing light or shadows. The light is not cut off at your ordinary clear-light viewing distance: unobstructed line of sight and the source’s actual light radius decide this temporary fringe appearance. Normal sight still decides full detail and exploration. This feature is not yet delivered.

## Loose Raw Essence crystals — installed in384

**Current behavior:** Loose Raw Essence now appears as an asymmetric blue crystal cluster, replacing the droplet. In current sight it has intermittent icy sparkles and a subtle blue aura. Remembered crystals are dim and still; undiscovered crystals stay hidden. The effect does not illuminate nearby terrain or extend sight. Collection, quantities, inventory and saved progress keep their existing rules.

Build384 installation and ordinary launch are verified. Focused internal checks and art review support the crystal and its current-sight and remembered appearances; physical-phone gameplay and Aimee’s aesthetic acceptance remain separate.

**Decided intended and separate proposal:** Separately, rare Essence resource nodes are decided intended behavior and are not yet available. Writing the Essence word is intended to provide one node, with artwork distinct from a loose Raw Essence find. The approved Count ladder changes that promise to one, two, three, four or five nodes, without repetition stacking.

## Other resource words and gems — current explanation and accepted direction

Rich Ore and Gilded Veins are different existing Compounds: the first writes strong Iron and ordinary Gold; the second writes strong Gold and faint Crystal. Their actual pressure effects can influence suitable mineral and stone opportunities, so this is not a metals-only rule. Crystal-derived sourceless light can also satisfy the planned natural Essence-node preference. Old flat reward multipliers in historical data are not current rewards and are not being restored. Further Essence-yield bonuses remain unapproved. Geological secondary gems are now accepted separately; Essence sources do not supply them. A personal Compound follows its actual marks rather than a suggestive title. The current material catalogue includes useful Quartz, but a broader gem family and jewelry-production system are not delivered. The small useful secondary-gem system and a bounded richness increase to its chance are accepted. The first-pass catalogue and uses below are Design tuning; this does not turn every deposit into a guaranteed gem reward.

## Geological gems — decided intended, not yet delivered

Suitable stone and metal deposits can occasionally contain one extra gem, collected with the ordinary material when you finish working out the source. It uses the same tool and action: no extra turn, gem skill or advanced shop is required to collect it. One source has one possible find; repeated hits and reopening do not roll again. Gems share the existing source tile and never replace required early materials.

**First-pass catalogue and tuning:** use existing Quartz plus Garnet and Beryl. Crystalline or granite-like stone deposits can contain any of the three; hard Iron deposits favour Garnet, Copper favours Beryl, and Silver/Gold deposits can contain all three. Loose Rubble, ordinary Quartz sources, Clay, Salt, Coal, flora and Essence do not gain a second gem roll. Each eligible source starts at four percent at Substrate peak25 or below, rising evenly to eight percent at100. These are first-pass chances, not a guaranteed find schedule. A success gives one gem. Richness changes chance, not quantity or item quality, and old Compound reward multipliers are not restored.

**A useful optional choice:** the first release pairs the new gems with optional inlay on Blacksmith gear that has a real metal fitting. Spend one gem during an otherwise normal craft to choose the small decorative stone in that fitting. The item keeps its normal power, quality, costs and unlocks; the extra ingredient is optional. Later suitable Bowyer, Weaponsmith and Armoury fittings follow their own coordinated crafting updates. No random gem is required for opening progression, tools, buildings or travelers, and this adds no separate jewelry shop.

Quartz retains its existing functional recipes. Garnet and Beryl start at the existing uncommon barter value band. All three are ungraded exact-name resources: colour and source history stay inside the stack and the chosen colour carries into its inlay. Ordinary refitting preserves the inlay; recycling returns its original gem once. No separate removal or replacement service is part of this first crafting feature. An inlay adds no automatic stat or resale multiplier and emits no light. Existing worlds and stock keep their original rewards. New drops will arrive together with their first usable fitting and finished inventory icons, not as unused collection clutter.
...
Learn the Essence Focus through a recovered lesson, then connect it to Substrate. A positive unmodified request guarantees one reachable node; approved Count words request up to five. Repeated requests use the greatest Count, and a naturally selected node counts within that total. The ordinary Page price still applies. The lesson can be tied to an existing loose Raw Essence find, so you need not discover a rare node before learning to request one. Existing saved worlds keep their sources.
...
**Current movement response:** Build388 makes the character and view respond together on the existing safe-step path to an already visible neighbouring tile. Selection, alert and work markers stay aligned with that scene. New details, saved world position and the next action still wait for the saved result; a failed save restores the character and view together. Steps involving encounters or other events can still take longer. This is a bounded camera-follow correction, not instant movement everywhere or a completed fix for every visual effect. Installation is verified. The earlier launch attempt was blocked by a locked phone; ordinary launch subsequently succeeded in390. Physical-phone appearance and interactive usability remain unverified. The misleading pending/completion ripple remains removed. Terrain and action costs keep their existing rules. Build396 checks covered a cue after input. Build399 delivers the corrected idle warning before input, with bounded campaign-copy verification. A completed crossing reports its actual total and extra turns; for example, crossing mud can take 4 turns, 3 extra. Ordinary-cost directions must have no slowing ripple; known adjacent slow directions should be indicated while idle, before input. Build397 reduces the delay in matched internal recordings, but movement responsiveness remains unfinished; physical-phone touch-to-visible-movement timing was not measured.

**Decided intended correction:** While your character is standing still, directional ripples should point toward every currently visible, reachable adjacent tile that takes extra turns to cross, before you press a movement control. No tap or pending action is required. Ordinary-cost and inaccessible directions have no slowing ripple, and hidden or merely remembered terrain reveals no new information. The ripples disappear when visible movement starts and return for the known surroundings after you stop. They add no delay, turn cost or saving-progress indicator. Terrain still costs its ordinary world turns, and saving still follows the normal action rules.

**Current objects and colour:** Currently visible, nondepleted resource nodes and loose finds have a restrained local animated glint so they are easier to identify. It does not promise that you have the right tool, can reach the source or have collected anything. Successful collection keeps its separate saved-action feedback and your existing message preferences. Plants and creatures retain their actual colours; readability fixes must not repaint every species alike.

**Current partial sight and memory:** Partially visible terrain may show a dim, still version of its own ground texture without revealing previously unseen plants, loot or creatures, marking it explored or adding discoveries. Remembered places keep their last-observed appearance; hidden changes do not refresh their resources or effects. Where an older explored tile has no appearance record, its already-known ground family receives a simple, still matching texture. That fallback does not recreate missing historical details, reveal neighbours or contents, or count as a new sighting. Existing world colours, movement costs, saved progress and fixed controls stay intact.

**Still intended and unfinished:** Reduce remaining movement latency and finish plant/creature visual polish, including exact fine-pattern appearance in remembered plants. Bounded internal checks and art review support the listed corrections, not every species or palette. Build382 installation and a subsequent ordinary launch are verified. Build383 is now installed; its ordinary launch was unavailable because the device was locked. This does not require an action from you or block development. No physical gameplay or smoothness claim is made.

## Explore presentation installed in382 — walking delay remains open

Build385 has removed the broad pending/completion ripple introduced in382. Build396 checks covered a cue after input. Build399 delivers the corrected idle warning before input, with bounded campaign-copy verification. A completed crossing reports its actual total and extra turns; for example, crossing mud can take 4 turns, 3 extra. Ordinary-cost directions must have no slowing ripple; known adjacent slow directions should be indicated while idle, before input. Build397 reduces the delay in matched internal recordings, but movement responsiveness remains unfinished; physical-phone touch-to-visible-movement timing was not measured. Movement responsiveness still needs improvement. Build382 also installs visible collectible glints, compatible static terrain textures and remembered plant colours. A modest neutral creature edge preserves source colours. Internal native checks and bounded art review passed; walking latency, exact remembered fine-pattern fidelity and overall plant/creature polish remain open. Build382 installation and a subsequent ordinary launch are verified. The later383 installation is verified; its ordinary launch was unavailable because the phone was locked. No physical gameplay or smoothness claim follows. Build381 adds a further saving optimization for existing World History entries. Their recorded facts, wording and appearance stay intact. Internal comparisons show a partial improvement in first-movement saving; remaining movement delay and slower first saves are still open. Build380’s earlier improvements remain: saving does less repeated work and the screen can keep responding while a move saves new discoveries. Both direction buttons and map travel wait for the accepted action to finish before accepting another move. If travel makes progress but cannot be saved, the party keeps its previous state and the message says Travel could not be saved; it does not report encounters, discoveries or later route warnings from the discarded trip. A route blocked before any progress keeps its ordinary explanation. Build379’s specific encounter-crash correction is retained. Movement costs, encounter order, withdrawal protection, saved progress and visibility rules stay unchanged. The earlier381 installation and ordinary launch were verified. Remaining movement delay and slower first saves are still open; this is not a claim of fully responsive walking or a physical-phone gameplay walkthrough.
...
**Decided intended — region placement during preparation:** When preparing a new world, the game may rearrange the shapes and positions of its ground regions to fit the required terrain, resources and routes. Each region keeps the rock, soil and mineral possibilities already established for it; moving its boundary must not reroll those qualities or copy a favourable region across the map. Conditions that depend on location, such as a nearby freshwater shore, must come from the final surroundings. Required materials still need real suitable sites and usable approaches. This concerns the unfinished world-construction overhaul; existing saved worlds keep their layout and contents.

**Terrain layers and amounts — decided intended clarification:** The new constructor preserves the material proportions assigned to the initial ground layout. Water, chasms, mud and growth then have their own roles, so those starting proportions are not a promise that the finished visible map contains the same number of Stone, Soil, Sand, Rubble or Ash tiles. Established resource promises and their usable approaches still have to survive.

Chasm influence shapes connected breaks and missing ground; it does not promise an exact final number or percentage of chasm tiles. Water and broken ground must be planned together while retaining the meaning of the Page. A difficult route is not permission to fill the gaps, flood away requested broken ground or remove a promised source just to make preparation pass. The exact water allocations retain their separate rules. No additional final terrain quota is being introduced by this clarification; complete construction remains unfinished.
...
**Verification:** The correction has passed three focused internal checks covering actual control actions, unchanged saved state, terrain rules and the existing collapse forecast. It is awaiting phone delivery; build382 does not include it.
...
**Verified scope:** Two focused internal checks passed and installation is verified. A physical-phone playthrough remains unverified. Existing terrain, mist, rain and the interim creature-material icons remain included; falling snow and mixed rain/snow are delivered in383; further icon polish remains unfinished.

## Terrain, mist and rain — installed; remaining atmosphere separate

**Current startup status — build383:** Build383 is installed with the sight corrections and Snow/mixed weather described above. Its ordinary phone launch was unavailable because the device was locked; development continues without an unlock request. It retains the earlier movement, saving and startup fixes. Build382 was subsequently launched successfully before this update, but no physical-screen or gameplay inspection was performed. Remaining walking delay and the earlier intermittent opening report are not declared universally resolved.

**Current behavior — installed in368:** Build368 adds the existing terrain textures to normal 3D ground and liquid surfaces, including grass, shallow water, deep water and solid ice. Their detail follows the world’s existing colours. Snow and settled ash keep their separate patches. Moving surface detail does not move the ground or change a turn. Remembered terrain keeps its last-observed appearance without live animation; build382 adds a compatible static family texture when an older explored tile lacks an appearance record. It does not reconstruct missing historical detail or reveal hidden changes.
...
**Sight edges in 3D — installed in383:** Build383 restores profile-based dimming across remembered terrain and objects, opaque black unknown ground, and an outward fade across never-explored fringe terrain. Actual obscuring air softly reduces detail in the admitted terrain texture; clear full sight remains sharp, and ordinary darkness adds no blur. Saved colours and last-observed objects remain intact. Sight distances, targeting, movement, camera and fixed controls are unchanged.

**Weather and sight — current versus intended:** Normal mist and rain are visible, but visible mist does not yet establish the intended reduction in sight: the current sight calculation still uses the older smoke-based information. The decided rules give the selected suspended medium its own recorded density contribution to sight; completing that connection is separate from restoring the 3D edge effects. Rain, falling snow, mixed precipitation and wind remain visual only for sight and add no damage, freezing or movement penalty.

**Recorded atmosphere and sight — decided intended:** The planned correction uses the weather already recorded for that world. Its selected smoke, airborne ash, mist or miasma supplies one density contribution to sight; a recorded clear atmosphere contributes none, even if an older record mentions smoke. Worlds without the newer weather record carry forward valid recorded smoke or remain clear, without reconstructing weather from old words. Rain, snow, mixed precipitation and wind add no sight penalty. Existing worlds keep their explored map and discoveries while current sight follows their recorded atmosphere. Newly prepared worlds must apply the same sight conditions from their first view, rather than revealing a larger area before the weather takes effect. The existing light, sight-bonus and density rules remain unchanged. This correction is decided and not yet delivered.

**Snow and mixed weather — installed in383:** Build383 adds falling Snow and mixed rain/snow to normal expeditions that already carry those weather conditions. Four coverage levels keep compact, slower Snow flakes distinct from faster Rain streaks, including both beside independent Mist. Effects retain their phase as the view is restored and remain confined to currently visible or fringe ground with supported geometry. Remembered-only and unseen places receive no live weather. This adds no accumulation, damage, freezing, movement penalty or new mode.

**Verification and remaining work:** Internal checks and shared native art review passed for the named sight corrections, Snow, mixed precipitation and Mist coexistence. Build383 installation is verified; ordinary phone launch was unavailable because the device was locked. That does not require an action from you or block development. Physical-phone appearance, playthrough and universal animation smoothness are not claimed. Remaining walking latency, fine-pattern fidelity and broader plant/creature polish stay open. Smoke, airborne Ash and Miasma presentation and the separate selected-medium connection to sight remain unfinished.
...
Build364 adds remembered non-enemy discoveries, consistent material totals with expandable exact variants, saved-reward gathering feedback, shared source placement and careful overlap repair. It also introduced the blue Raw Essence droplet, since replaced by the crystal cluster in384, the Tutorials switch, clearer opening guidance and square inventory quality frames. Terrain colours, fixed controls, supporting land faces and the earlier plant, Diary and waterfall corrections remain intact.

Installation and ordinary app launch are verified. The focused internal checks do not amount to a physical-phone playthrough or your final visual acceptance. Raw Essence was collected and stayed collected after reopening in an ordinary internal expedition. The replacement crystal’s dim, static remembered appearance subsequently passed internal review for384.
...
**Current Raw Essence — installed in384:** Loose Raw Essence now appears as an asymmetric blue crystal cluster, replacing the droplet. In current sight it has intermittent icy sparkles and a subtle blue aura. Remembered crystals are dim and still; undiscovered crystals stay hidden. The effect does not illuminate nearby terrain or extend sight. Collection, quantities, inventory and saved progress keep their existing rules. Collecting it adds the actual quantity and removes the find; reopening preserves that collection. Raw Essence remains separate from spendable Essence and Motes. Its remembered appearance passed internal review. Separately, rare Essence resource nodes are decided intended behavior and are not yet available. Writing the Essence word is intended to provide one node, with artwork distinct from a loose Raw Essence find. The approved Count ladder changes that promise to one, two, three, four or five nodes, without repetition stacking.
...
**Relief preservation — decided intended, constructor unfinished:** The new world constructor must keep the amount and character of raised terrain called for by Relief while building usable approaches around its cliffs. It may rearrange an unfinished layout and add stepped routes; it must not make a difficult world pass by flattening away the requested high ground. The amount of initially raised terrain is set before water and other terrain features are added. This is separate from the access rule: at least 90% of the final dry terrain above the lowest dry level must be reachable from the entrance. It does not promise access to 90% of the whole map or to deep water. The exact numbers of tiles at each individual height are generated, and existing saved worlds keep their terrain.
...
**Optional field notices — current in build 320:** Monster notices and mining/gathering results can be switched separately. Muting them changes the popups, not the actual world, resource collection, map labels, Diary or inventory. Current visible Apex identification remains until the accepted aura/red-dot correction above replaces map and minimap badges; ordinary inspection names remain useful.
...
**Current written promise; the cold-world change below remains intended.** Deliberately writing Iron into Substrate promises **one cluster of two reachable Iron deposits**, provided the complete promise can be fulfilled. A compound saying the same thing counts too. Repeating the request or changing Count, Scale or Intensity does not multiply the guaranteed cluster; those words keep their existing effects on the world and writing costs. Iron left for the world to choose does not receive this written guarantee. Absent Iron, Iron bound to another subject, or an Iron statement that negates its own Substrate effect does not qualify. Negating one statement does not cancel a separate positive one.

The two deposits use the existing Pick 1 rule and contain twelve Iron altogether. They satisfy the ordinary Iron reservation once and do not create duplicate bonus deposits. Current generation counts them within its mineral budget. Under the decided new preparation rule above, required sources establish the minimum allocation while the ordinary target limits optional additions.

**Current host rule:** Use existing suitable exposed rock first. Where necessary, the world may expose compatible rock at no more than the two deposit sites in otherwise empty, dry, unfrozen ordinary soil. Both deposits belong to one compatible region. This cannot drain water, thaw ice, change heights, remove a source or object, or overwrite protected arrival, teaching or return routes. The exposed ground stops counting as growing soil.

**Decided intended for newly constructed worlds — not yet delivered:** Cold alone will no longer prevent the promised Iron cluster on otherwise suitable dry exposed rock, including the existing limited exposure of at most two ordinary-soil sites over compatible bedrock. This does not thaw ice or make submerged faces usable. Both deposits still share one compatible region, keep their reachable working positions, require Pick 1 and yield twelve Iron altogether. The world remains cold; ordinary mineral and plant rules and existing saved worlds remain unchanged.
...
Before Binding, the quote identifies the complete promise and the known tool requirement. Current preparation refuses before spending if two reachable deposits cannot fit; it does not silently promise just one. That refusal protects the player while a preparation defect remains. Under the decided construction rules, a valid supported Page must instead receive its complete lawful cluster. A genuine conflict between direct written guarantees follows the existing compatibility rule: the quote names which promise wins and which remains an influence before you Bind. Existing worlds keep their original contents. This first binding adds no new writable Coal, Clay or Resin vocabulary.

**Development check:** selecting a collected Iron-written Page, reviewing its quote, Binding and entering the world succeeded in the native app. The quote promised two reachable deposits requiring Pick 1, and the resulting world kept that promise. This test used an arranged campaign; it does not establish the natural route to learning Iron or making your own first Iron page. Direct and compound Iron statements, incomplete promises, changed quotes, saving and reopening have separate rules checks. This is the historical development receipt, not a new delivery or natural-progression check.
...
**Current host rule:** Use a suitable exposed crust first. Otherwise, the written material may form one small crust on empty, dry, unfrozen soil or sand with known ordinary-water chemistry. The surrounding world can remain fresh: this request does not require a salty-water climate or a lucky ordinary Salt roll. It preserves water, heights, underlying ground and protected sources, and cannot drain a pool, thaw ice or clear a path to make room. The crust’s own patch cannot also be a growing plant.

**Decided intended for newly constructed worlds — not yet delivered:** Cold alone will no longer prevent the promised Salt Crust on otherwise suitable dry, exposed soil or sand. The world stays cold, and its plants retain their actual growing conditions. Ice-covered or submerged sites remain unsuitable; the deposit still needs the same ordinary-water chemistry, empty location outside protected sites and the same three-to-eight-step route. Its one-source promise, hand gathering and two-Salt yield do not change. Ordinary Salt keeps its existing habitat rules, and saved worlds keep their contents.

**Current allocation and preparation:** The promised deposit uses one existing earth-gathering place before ordinary Clay and Salt allocation, and counts toward the ordinary Salt reservation. It is not extra stock or an enlarged source budget. Before Binding, the quote must confirm the reachable hand-gathering deposit. Current preparation refuses without spending if no legal site or budget fits. That remains a safeguard while a defect is unresolved, not the accepted final outcome for valid Writing. The decided construction and required-population correction above must satisfy the promise on lawful hosts. Only a genuine conflict with another direct written promise follows the existing disclosed winner-and-influence rule; a failed placement cannot silently become pressure only. Older books keep their existing behavior. This introduces no new word, free knowledge or phone-delivery claim; ordinary Salt already works independently in development.
