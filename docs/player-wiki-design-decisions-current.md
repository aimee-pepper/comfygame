...
**Written Salt remains unsettled:** the general intended resource promise is one hand-gathering source, but the exact Salt statement that makes that promise—and what happens if its physical source cannot fit—still needs a specific rule. The ordinary Salt source does not depend on that future Writing route. The tested written-Iron rule must not be read as proof that written Salt already guarantees a deposit.
...
Animal frequency, source matching, combat, and the complete gathering journey still need playtesting. Ordinary Salt is tested in development as described above; animal-source changes, Leather processing and the Leather Guard are not established as complete by that milestone. The wider creature rework is still unfinished, including additional body-derived materials, their uses and broader ecology. This Leather work does not update the Library Bestiary or establish that the whole creature rework is complete.
...
The selected original resource pictures will keep their original colours, framing and size; only their backgrounds are being made transparent. The prepared Coal, Hardwood Log, Leaf Fibre and Softwood Log pictures have not yet been integrated into the native game or accepted on the phone.
