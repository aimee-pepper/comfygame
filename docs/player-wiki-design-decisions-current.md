...
**Current behavior and reports:** The slow-terrain cue, actual-cost notification and Torch light are installed, and ordinary launch succeeded in395. A subsequent phone playtest reports that the Torch light still trails the character by a step, the ripple is missing before the slow crossing, and it intermittently appears during movement. These timing defects remain unresolved; the earlier internal checks did not establish correct timing during ordinary play. The actual-cost notice was verified in a native four-turn crossing. The Torch source must follow the displayed character within existing sight limits, and the ripple must clear at the first visible movement without adding a pause. On affected steps, the character, view and visible illuminated area will advance together once the next saved view is ready. Steps that can respond early without a mismatch keep that immediate response. This adds no timer or extra turn and reveals no new surroundings before the saved result. Reducing the underlying preparation delay remains part of the correction.
