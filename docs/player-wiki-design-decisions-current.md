...
**Current behavior:** Movement has a reported blocking failure: all movement directions can stop responding, even after reloading. Restoring movement while preserving the campaign is the immediate priority. Delayed movement from the current light-synchronization approach also remains unresolved. Earlier internal checks showed some matching movement and lighting, but did not establish the responsive experience required. Correcting this must preserve prompt movement and a matching lit area together.
...
## Gameplay notices — corrections pending

**Current behavior:** Internal world-correction notices can still interrupt Explore, and a reported attempted gathering refusal underfoot can be silent. Both corrections are pending; earlier checks of target labels do not establish that the attempted action explains its refusal.
