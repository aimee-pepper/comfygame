...
Installation and ordinary launch are verified. This update changes interaction labels only. Camera-follow coherence, the Apex aura and minimap dot, moving both information panes together, portal and site artwork, the reported grey appearance of plants and creatures, and moving lights remain unfinished.

## Movement response — installed in386

Build386 gives earlier visible character movement for eligible safe steps to an already visible neighbouring tile. It does not reveal hidden details early. Discoveries, world changes and the next action still wait for the saved result; a failed save restores the character’s saved position. Steps involving encounters or other events can still take longer, and accepting the next move still waits for completion. This is a partial responsiveness improvement, not instant movement everywhere. Installation and ordinary launch are verified; internal timing measurements are not a physical-phone latency guarantee. When the map needs to pan, it can still follow the character a half-step late. The intended correction makes the character and view respond together using only already-visible scenery; it does not reveal new details early and is not yet delivered. The misleading pending/completion ripple remains removed. Terrain and action costs keep their existing rules. A truthful stationary slow-terrain cue remains unfinished because movement has no separate exposed slow phase.

## Apex warning and readable depth — intended corrections

Visible Apex creatures should have a threatening aura around their bodies in the main world view. The minimap should show only a red dot, or a slightly larger red dot—no Apex text, badge or aura. This restores the discussed warning treatment; it does not change combat or reveal hidden creatures. The oversized labels are a reported current defect, and the correction is not yet delivered.
