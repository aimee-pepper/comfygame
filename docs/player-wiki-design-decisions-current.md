...
**Current:** Build398 improves some internal movement measurements, but the current phone report still identifies a lit field trailing the character by about half a second and no visible slow-terrain warning before input. Both defects remain open. Earlier checks of a ripple after a move was requested did not verify the requested idle warning. Installation and ordinary launch succeeded; the measured improvements do not establish smooth or correctly aligned movement in the reported campaign. Current gathering can open Choose tools even when a suitable tool is available, and an adjacent source can incorrectly take priority over the source under your character. These reported defects remain open.

**Decided intended behavior:** While your character is standing still, directional ripples should point toward every currently visible, reachable adjacent tile that takes extra turns to cross, before you press a movement control. No tap or pending action is required. Ordinary-cost and inaccessible directions have no slowing ripple, and hidden or merely remembered terrain reveals no new information. The ripples disappear when visible movement starts and return for the known surroundings after you stop. They add no delay, turn cost or saving-progress indicator. The character and its currently visible bright area must stay aligned on every movement path, with or without a Torch and whether the view pans or stays fixed. Movement must preserve the existing sight boundary: no new surroundings appear before their saved result. A step that cannot safely respond early must publish its matching character, view and sight together as soon as ready, without an artificial pause. Gathering should automatically use an applicable tool already available to that action. A valid chosen source takes priority; otherwise the live source under your character comes first across resource types. If nothing is underfoot, a workable adjacent source comes before one whose requirement you cannot meet. The action label, highlighted source, requirement and used tool must agree. This adds no equipment swap, purchase, extra turn or automatic site or portal use.

**Decided target selection:** Decided intended interaction: hold Interact, slide to a nearby resource and release to select that target. Release outside to cancel; the next tap performs the labelled gathering action. Selecting a target will not harvest anything. The existing hold-to-select-tool gesture remains current until its replacement is delivered.

These corrections require no repeated decision or homework from Aimee.

## Slow-terrain feedback and coherent movement

**Current behavior and reports:** Build398 improves some internal movement measurements, but the current phone report still identifies a lit field trailing the character by about half a second and no visible slow-terrain warning before input. Both defects remain open. Earlier checks of a ripple after a move was requested did not verify the requested idle warning. Installation and ordinary launch succeeded; the measured improvements do not establish smooth or correctly aligned movement in the reported campaign. In the reviewed movement sequences using normal controls, the party stayed visible, a known slow-terrain cue appeared while stationary and was absent by arrival, and destination details and the crossing notice followed arrival. Newly entered tiles no longer appeared black in the three inspected crossings. These historical samples do not close the current idle-warning or general lit-field defects. Installation and ordinary iPhone launch succeeded. A separate phone material check confirmed the lighting response, without interacting with campaigns; appearance and movement in a normal campaign on the phone remain uninspected. The rule still requires the ripple to clear at the first visible movement without adding a pause. The character and its currently visible bright area must stay aligned on every movement path, with or without a Torch and whether the view pans or stays fixed. Movement must preserve the existing sight boundary: no new surroundings appear before their saved result. A step that cannot safely respond early must publish its matching character, view and sight together as soon as ready, without an artificial pause. The existing correction does not yet cover the reported ordinary movement path.
