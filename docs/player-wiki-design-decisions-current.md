...
**What has been checked:** Engineering’s native Simulator journey collected both lessons, returned Home, read them, reopened the game and reached a usable joined-Sun Bind quote. Rules checks also completed that authored expedition and read its Faint/Moderate lessons, and covered practical lesson pairs, missing Subjects, traveler rules and a material → recruit → Apothecary → Salve connection with saving failures. This is bounded evidence, not a fully natural multi-expedition native playthrough or final pacing acceptance. Build331 installed successfully; automatic launch was refused because the phone was locked. Your visual and play feedback remain separate.
