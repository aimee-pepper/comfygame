...
For the upcoming recording playthroughs, newly generated ordinary2D worlds will omit the unfinished box deposits for **Iron, Coal, Quartz, Sulfur, Mercury and Rift Glass**, including equivalent unfinished deposit sources. Existing recognizably rendered sources remain; this is not a removal of every resource without a recent art review. Work on new2D resource art stops;3D node graphics take priority. The temporary plan to use an ore picture for Iron is superseded and does not count as finished artwork. Inventory pictures alone do not establish finished map-node graphics.

Trees, Clay, Salt Crust, plant and root gathers, and ordinary rubble ground remain. Existing saved worlds and earned stock stay unchanged. A world's contents will not appear or disappear when its renderer changes. Worlds generated under the3D policy keep their resource-node capability. This is a temporary generation restriction, not a new trial or player mode.

**Temporary progression consequence — accepted for recording:** unavailable Iron and Coal deposits may block Iron-learning, Forge, Ingots and metal-tool progression; the other excluded minerals also limit their specialist crafts. Recording the pre3D appearance is the purpose, so no alternate progression route is being added. Ordinary worlds will drop the conflicting deposit reservations so they can still generate. If explicit Iron writing promises deposits that this policy cannot place, Bind will explain **“Iron deposits are temporarily unavailable in2D”** before spending, instead of failing as a save error or silently breaking the promise. Salt writing and retained gathers keep their existing rules. No free or substitute materials are granted, and existing owned materials remain usable. The scope is settled; implementation is still pending.

**Encounter crash — fixed on your phone in343:** you confirmed the encounter now works after the further correction in build343. This closes the crash reported on342. Your campaign was preserved. The temporary2D deposit restriction and new3D deposit artwork are not included in343.
...
**Mining prompts — decided, awaiting delivery:** routine mining feedback will be one short cue. If you own a suitable tool but have not selected it, the prompt names it: **“Select your Stone Pick.”** If you lack a strong enough Pick, it states the minimum: **“Requires Pick Lv 2+.”** Other blocked actions explain the actual obstacle briefly, without incorrectly asking you to switch tools. Hidden deposits reveal no material or tool requirements. Mining rules stay the same; longer messages can still wrap when needed. The installed mining prompt is still the longer version you asked us to shorten.

**3D deposits — current appearance and artwork progress:** build343 still shows unfinished grey boxes for these deposits in the3D view. Distinct models for **Coal, Iron, Quartz, Sulfur, Mercury and Rift Glass** are now complete and internally reviewed at ordinary map scale, but are not yet confirmed delivered to your phone. Mercury appears as solid rock with silver exposed faces; Sulfur forms a low yellow crust, while Rift Glass has broad opaque shards. These appearances keep the existing mining requirements and rewards. The temporary2D spawning restriction remains pending and is not changed by finished3D artwork. Build343 retains the earlier Bind/departure corrections below.
