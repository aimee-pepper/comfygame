...
## Apex and solid sides — installed; chasm depth — intended
...
Build390 darkens textured solid terrain sides to distinguish them from their tops, preserving their source hues and textures. Top surfaces, terrain geometry and existing sight dimming keep their rules. This is a side-contrast correction, not a new lighting system. A discovered chasm should show the adjacent terrain’s walls descending into darkness, with about two levels of visible descent as first-pass artwork tuning. That is not a physical bottom or a new walkable floor. Keep unknown places black and remembered edges limited to what was last seen. The decided chasm-outflow extension allows an actual flowing channel to end at its recorded open lip and fall into darkness, without a receiving pool. Still water, ice and an unselected or blocked edge do not automatically spill. This applies to newly prepared worlds under a saved policy, preserves water amounts, and requires the source and chasm boundary in full sight for live flow; memory stays still and last observed. Chasm walls and their waterfalls remain planned; they do not wait for a change of rendering system.
