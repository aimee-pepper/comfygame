...
**Creature design requirement:** complete the structure rules for all seven accepted forms—Four-legged, Two-legged, Serpentine, Segmented, Radial, Fish-shaped and Amorphous—across Sky, Water, Amphibious and Land before building dependent creature generation. The existing compatibility rules allow23 habitat/form combinations. Body structure, supporting legs, additional appendages and head features must agree; a wing does not automatically grant flight, and fish-shaped shore creatures need supporting limbs. The current seven-form structural draft is ready for reference and Engineering review. Your Dragon DNA game remains the intended inspiration for independently combining detailed features;23 habitat/body pairs are not a cap on that granularity. Its reference must be reconciled before the team calls the creature design complete. This is not a claim that the generator, artwork or broader ecology is complete.

**First-pass proposed tuning:** size and build change proportions within each family; stable shapes, coverings, colour and Pattern create variety without changing species on reopening. Exact dimensions, component proportions and pattern treatment remain revisable production choices. The first flora presentation slice covers source-backed woody trees and low fibrous growth; other forms keep their honest existing presentation until supported. Engineering must confirm the full creature contract before family-specific implementation; reusable interfaces and independent flora/water work can proceed.

**Waterfalls need a real connection:** an elevated pool or channel must have an open outlet leading down into actual receiving water. Both local water levels, the connecting edge and the exposed drop must be generated and saved. Separate pools keep their own levels; overlap on screen, a lower pool behind a ridge, frozen water or a blocked outlet does not create a waterfall. The initial proposed policy uses drops between already non-walkable deep-water cells, preserving the opening’s routes. Numerical depths and the visible-drop threshold are first-pass tuning under Engineering review, not measurements being guessed for old worlds.
