...
**Reusable flora artwork — current in348, newly bound books in the existing3D view:** broad woody leaves and branches now use the reviewed Blender artwork, assembled using each plant’s existing size, arrangement, source colour and saved structure. This adds no new species type, organ, harvest or growth rule. Newly bound books use this first component pair; existing books and saved plants keep their recorded appearance. The normal2D view is unchanged. The next woody trunk component has passed internal visual review, but its rollout is still pending and it is not included in348.
