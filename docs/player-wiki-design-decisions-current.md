...
**Material inventory artwork — unfinished:** replacement icons must match the pixel-art style already used in game. The first prepared set was too finely textured, so its new artwork is being revised against the existing icons. Matching appearance, usable transparent images, integration and in-game readability remain pending. Preparing icons does not resolve the separately reported stacking issue; the grouping rules above remain unchanged.
