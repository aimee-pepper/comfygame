...
**Current clarity issue:** Return does not yet list the newly earned lesson names among permanent gains. The lessons are saved and can be read in the Library. The pending correction adds a **New lessons** list with **Ready to read** or **Read** beside each saved lesson and points to the Library at Home. Receiving a lesson and learning its word remain separate; no learning or reward is missing.
