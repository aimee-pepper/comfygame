...
**Current behavior:** The movement regression remains open: the current synchronization approach can make walking feel delayed. Earlier internal checks showed some matching movement and lighting, but did not establish the responsive experience required. Correcting this must preserve prompt movement and a matching lit area together.

**Decided intended behavior:** Movement must respond immediately while the character and its visible lit area move together. Delaying the character until the light catches up is not an acceptable fix. This applies with or without a Torch and whether the view moves or stays fixed, while keeping the existing discovery, saving and turn-cost rules.
