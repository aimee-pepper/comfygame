...
**New proposal for the water demonstration — awaiting your decision:** use a small, separate terrain study with deliberately chosen example heights: a shallow pond on raised land, a lower channel, and dry ground between them. The pond and channel would each have their own water level, with visible beds only where the study permits them. This would be clearly labelled as an example scene, not a generated expedition. Existing saves do not contain measured water depths, so the study would not assign invented depths to your worlds or change your campaigns. The existing-world camera trial can continue independently.

The first water study would be viewed from a dry, level route. It would demonstrate appearance, not prove that new slopes or shore routes work. The agreed movement rules still need implementation before an interactive height-crossing demonstration. Numerical depths for generated worlds remain design work; the study does not establish a universal wading depth. A visible water surface would never, by itself, reveal an unknown bottom, deposit or creature. See [Aimee Homework](aimee-homework.html) for the one decision needed to add this study.
