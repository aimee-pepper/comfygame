...
**Decided behavior:** Walking onto a place must never hide or remove it. Sites stay in the world with their committed unlooted or looted appearance; entering or starting a search does not complete it. No intermediate looted appearance is being added now. A canopy or roof may soften only where it blocks the character, while the actual place stays visible. Portal stone is fully opaque; only the blue magic may be semitranslucent. In the planned local-light system, the solid stone frame blocks light while the blue veil and moving magical ribbons let it pass. This changes no portal artwork or travel rule; that local-light behavior is not yet delivered. A loose object can leave the ground when it is actually collected, including an existing automatic pickup during a successful step; simply standing over an uncollected object must not hide it.
