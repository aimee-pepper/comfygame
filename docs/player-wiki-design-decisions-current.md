...
**Current behavior:** the existing Settings3D trial has its delivered tree presentation and neutral creature markers. The new generated assemblies and real connected-water waterfalls are not delivered. The previously approved terrain/water example remains a separate study, not evidence that generated worlds already have measured water depths.

**Creature design requirement:** all intended forms must have coherent structure rules before dependent generation is built. The existing seven body labels have23 habitat/body combinations, but that is only the starting classification. Dragon DNA’s independently varied wings and limbs, horn style/direction, spine style/height and tail shape/length are the model for the deeper modular design. The reference has now been read, and the draft replaces the restrictive single-appendage choice with independent part groups. A six-legged creature should not be mislabeled Four-legged; a **Many-legged** subgroup is a proposed classification refinement. The independent structures and their source rules have passed feasibility review and now have a tested development implementation; ordinary-world enablement and delivery remain pending.
