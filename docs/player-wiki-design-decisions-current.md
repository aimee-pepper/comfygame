...
**Next reusable flora artwork — intended, rollout pending:** individual broad leaves and branches have been authored in Blender and assembled in internal tests using each plant’s existing size, arrangement, source colour and saved structure. The first pair improves woody plants; it adds no new species type, organ, harvest or growth rule. The artwork now loads in the game’s internal test view and has passed its visual review. It is ready to be introduced into newly created worlds through a planned rollout. Ordinary world generation has not changed, and existing saved plants keep their recorded appearance.
