...
**Implemented and internally reviewed, installation pending:** the selected source now has its action text and a small base outline when ready; visible creatures can show their actual Apex and alert status. Success cues follow saved work and your notice settings. Several nearby sources do not cause an arbitrary target to be chosen. The existing view admits creature labels only at full sight. The reviewed examples passed their visual checks, while every possible overlap and the final cleared-source symbol have not been visually checked. Existing danger and tree-fading rules are retained. This introduces no new test menu, knowledge requirement or decision homework.
