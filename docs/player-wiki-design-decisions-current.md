...
**Current behavior:** the 3D view has a static warning at an actually harmful plant's base, visible only while that patch is fully in sight. The warning was included in the installed build310; the old note that it was absent from307/308 is historical. Actual Apex labels are delivered on the existing field, minimap, inspection and encounter views, but the 3D creature artwork does not yet carry the same badge. Its general resource markers also do not yet show complete selected-action readiness.

**Decided intended behavior:** the 3D view should make the same lawful distinctions clear. A harmful patch gets one generic **Dangerous growth** cue from first full sight; it needs no prior injury or learning. An actually visible Apex gets its own Apex distinction. Size, stillness, colours, thorns or impressive artwork do not create either status. A distant warning keeps its existing limited meaning and does not reveal a creature's body or materials.

Select the source's actual base, not its overhanging leaves or body parts. With a suitable packed Axe and a legal working position, a tree's base highlight and **Chop tree** identify the action. Other sources keep their own tools, reach and action rules. A visible source may still require a different tool or approach; its selected explanation should say why. Success feedback follows the actual saved action, never a cancelled attempt. Source colours remain intact beneath separate warnings and selection marks.

**Memory and fading — current rules retained:** remembered trees keep their last observed appearance and can fade when covering you. They do not disclose current hidden harvests, danger, enemies or changed shadows. Moving creatures do not leave a remembered body behind. Fading restores the view of your character using only information already allowed by sight; it does not reveal a hidden target.

**First-pass presentation plan — not delivered:** keep a small selected-base outline, a separate static growth warning and the existing Apex badge vocabulary, with action text on the selected-action view. The missing 3D status and action connections still need implementation before final artwork. Existing tree fading is already set; this plan does not reopen its timing. No new player trial, knowledge requirement or decision homework is added.
...
**Decided notification direction — remaining changes pending:** routine notices will give a brief action, result or actual reason for refusal. Quantities, relevant requirements and necessary choices remain visible. Full diary passages belong in their existing Library records, and measured survey values remain in the world's existing Measured details. The notice confirms the event without repeating the whole explanation. Combat logs keep actual targets, effects, amounts and durations. The broader wording audit is complete; its remaining changes are not yet delivered.
...
**Unread Findings — decided, not yet delivered:** the Library will have a distinct **Unread Findings** section, showing a count and lesson titles. Return's **Read findings** shortcut will open that section directly. For now, this section covers recovered lessons; it does not collect every unread Bestiary or history entry. Opening a ready lesson teaches it without Essence or a world turn. A lesson leaves the unread list only after learning is saved. If a Subject lesson is required first, its name is shown. Learned records remain available in Field Notes and learned words in the Dictionary; unread lesson tiles will no longer be duplicated in Field Notes.

**Current lesson route:** the installed **Read lessons** shortcut still opens the Library's main page. To learn Wildfire now, open **Library → Field Notes → Wildfire**. Opening the lesson applies its learning; there is no extra Read button. In the opening progression, Wildfire requires any one of **Hydrology, Thermal or Vitality** to be known first. A recovered lesson is safely stored before it is learned; the current **Ready to read** label does not reliably distinguish that prerequisite. The dedicated section and truthful readiness labels are pending.

**Material stacks — decided, not yet delivered:** materials of the same kind and quality share one visible stack. The reported8 and2 units therefore appear as **Common Supple Hide ×10**. Their nuanced properties, colours and exact source records stay stored; clicking the stack expands those details and variant quantities. Those differences do not split the main list or clutter its collapsed rows. Source IDs, raw colour numbers and unfinished-rendering messages will be removed from player details. Crafting still uses the actual materials and their preserved properties, without averaging them.

**3D deposits — artwork complete internally, delivery pending:** the installed3D view still uses grey placeholders for these deposits. Distinct models for **Coal, Iron, Quartz, Sulfur, Mercury and Rift Glass** are internally reviewed at ordinary map scale. Mercury appears as a solid mineral-bearing host with silver exposed faces; this does not add a liquid-pool interaction or change its mining rewards. Sulfur has a low yellow crust, and Rift Glass has broad opaque shards. Finished3D artwork does not change the temporary2D restriction. Build344 retains the earlier Bind/departure corrections below.
...
**Return lessons — current in build332:** Return now lists **New lessons** earned in that expedition, using their Library names and **Ready to read** or **Read** status. **Read lessons** completes the normal Return acknowledgment and opens the Library at Home; a failed save keeps that transition from completing. Reading remains a separate action that learns the word. Already-earned lessons from other expeditions and unearned offers are excluded, and no section appears when there are no new lessons. Engineering supplied four focused checks and an inspected native render using a separate copy of the preserved campaign; no extra expedition was run. Build332 installed and ordinarily launched successfully. Your visual acceptance remains separate.
