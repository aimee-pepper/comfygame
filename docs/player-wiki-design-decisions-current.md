...
**Encounter correction — installed in343, physical encounter result still unconfirmed:** build342 did not resolve the crash you reported. Build343 includes a further correction and is installed with a successful ordinary phone launch. Internal checks using the previously failing campaign passed movement into an encounter, opening Techniques and reopening the saved game. Your campaign was preserved; no phone gameplay was performed, so the physical encounter crash is not yet confirmed resolved. This update includes neither the temporary2D deposit restriction nor the new3D deposit artwork.
