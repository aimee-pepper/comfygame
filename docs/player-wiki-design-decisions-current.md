...
**Current behavior — installed in build333:** newly written overhaul worlds now use generated flora and creature assemblies in the existing 3D trial. Connected water receives measured surfaces, shore slopes and short steps only where every existing walkable crossing can be preserved; unsupported water keeps its previous presentation. Native testing passed movement, generated encounters, saving and reopening. A controlled example verified a real drop between connected water levels; the ordinary expedition tested had no qualifying waterfall. Existing saved worlds retain their original generation and appearance. Working shapes and static falls remain partial visual progress, not finished artwork or proof of natural waterfall frequency.

**Creature design requirement:** all intended forms must have coherent structure rules before dependent generation is built. The existing seven body labels have23 habitat/body combinations, but that is only the starting classification. Dragon DNA’s independently varied wings and limbs, horn style/direction, spine style/height and tail shape/length are the model for the deeper modular design. The reference has now been read, and the draft replaces the restrictive single-appendage choice with independent part groups. A six-legged creature should not be mislabeled Four-legged; the accepted **Many-legged** subgroup describes creatures with more than four support limbs. The independent structures and their source rules have passed feasibility review and now have a tested development implementation; ordinary-world enablement and native play verification are complete; build333 is installed and ordinary launch succeeded.
...
The water study uses a dry, level route; the later feedback above requests deeper example water, north-up orientation and all four directions on its connected flat walking area. It demonstrates appearance; interactive slopes and shore routes still need their own implementation. The agreed movement rules still need implementation before an interactive height-crossing demonstration. A separate first-pass policy now specifies generated depths and shared shore crossings, pending implementation; this earlier study does not establish a universal wading depth. A visible water surface would never, by itself, reveal an unknown bottom, deposit or creature. The approval is recorded as complete in [Aimee Homework](aimee-homework.html).
