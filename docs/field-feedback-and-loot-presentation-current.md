...
> **12 September reconciliation:** The [current Explore reference](player-wiki-design-decisions-current.md#explore-response-and-readability--restoration-in-progress) separates the requested directional movement ripple and idle collectible glint from the static consequence cues and successful collection described here. Those restorations await delivery; existing gathering feedback retains its current saved-action and notice-preference rules. Earlier candidate-only integration status and fixed-configuration review instructions below are historical development records, not current production holds.
