...
When the food policy is absent, retain the base role policy and its existing Grazer-disabled data; old legacy names remain saved. A valid new food policy supplies only the explicit supported-leaf-browser exception to that guard. Do not globally flip the old role JSON or backfill diet into earlier worlds.
