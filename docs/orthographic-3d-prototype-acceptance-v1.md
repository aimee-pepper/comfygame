...
This choice does not add stacked floors, bridge underpasses, physics simulation, off-grid movement, camera rotation, dynamic time-of-day or a day/night system. A fixed demonstration light is not a new lighting mechanic. No current game work is scrapped, no full-world asset conversion is requested, and no standalone Wiki publication is needed for this brief. If recorded publicly in the next cohesive update, label actual 3D rendering **under consideration**, distinct from the accepted three-quarter presentation and later shadows.
