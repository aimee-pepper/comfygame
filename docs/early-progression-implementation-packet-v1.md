...
## Unwritten intensity — adopted measured first-pass policy

**Decided intended tuning, not delivered or map-accepted.** New early-world generation selects unwritten intensities at **Faint70 / Moderate25 / Great4 / Overwhelming1** instead of uniform25/25/25/25. The whole source pool and all four non-absent intensities remain available. Obtain the same existing rolled source/target/ID candidates once; preserve every other sigil field. Player-authored choices are never reweighted, and their ordinary inherent cross-target effects remain.

Freeze the measured stream exactly: SHA256 of UTF-8 `early.intensity.candidate.v1|` followed by `target.rawValue`; take the first8 bytes as a big-endian UInt64 salt; use `SeededRNG(seed: worldSeed).derived(salt).double(in: 0...100)`. Values below70 select Faint, below95 Moderate, below99 Great, otherwise Overwhelming. Retain that experimental namespace literally for deterministic continuity; saved policy identity is `early.unwritten_intensity.v1`.

The fixed-source native pressure diagnostic on0–999 passed, retaining source/target/IDs and authored choices. With the corrected root baseline and no drainage, blank pressure passes rise67→306 without shore and60→270 with shore; Sun-moderate rises203→417 and167→384. These results are recorded in `test_sim_2026-09-05T01-47-37-696Z_pid44441_bc3e31aa.log` (2.987s). At the report, the diagnostic was an uncommitted continuation of Engineering86cc7ca7; Engineering must preserve the exact test revision before the next handoff. Do not present that working native source as delivered implementation.

Use this saved policy consistently in new pre-Bind quotes/previews, actual Bind, recorded rolled sigils and causal evidence. Newly silent targets in a same-seed intervention use this policy’s replacement; all unrelated actual rolled candidates remain frozen. Never reweight a player-authored sigil, a saved old rolled list, an existing world/anchor snapshot, or an old root receipt. An absent policy on an existing record preserves its previous generation behavior; it does not silently opt into these weights. Persist the selected policy before any successful Bind receipt.

All habitat thresholds, local water/root exclusions, geology, finite placement budgets, source reservations, material quality and authored guarantees remain unchanged. Dry-Loam drainage is not included. The experiment supports advancing to actual maps, not declaring the25% host floor passed.

Before another full1000-world corpus, run fixed seeds0–63 for blank and Sun-moderate inputs through actual new-policy terrain, regional facts and finite placement (128 worlds total). Separate pressure passes from regions with twelve suitable roots and legal Resin/Softwood sites, and from actually placed required sources. Report missing reservations on an eligible legal host as failures, exact generation refusals separately, and both authored cohorts separately. Inspect actual host loss before the costly full corpus. The gameplay gate remains off until connected native gathering/recruitment/Return/foundation/Salve/next-Bind and delivery acceptance pass.

## Actual-map follow-up: connected habitat scope

The next authoritative correction and measurement disposition are in [Connected habitat facts](early-connected-habitat-facts-v1.md). Aggregate the unchanged twelve-cell habitat requirement across eligible entry-connected regions for Nessa and Corrin, matching the existing flora budget; keep per-cell ecology and legal source requirements. Old saved facts remain frozen. The actual128-world sample has starter plant sources in16/64 blank and23/64 Sun worlds, while the narrower old Nessa fact counted13/18. Do not conflate them or retune climate from the smaller number. After focused aggregation checks, the next run is one actual blank0–999 corpus with both old and revised fact counts plus real material availability. The previous128-world prerequisite is complete as an evidence collection, with four diagnosed legal Clay capacity exceptions and no claim that its revised assertion was rerun.
