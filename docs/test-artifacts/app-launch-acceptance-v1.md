...
- Ordered transition evidence: `app-launch-transition-v1.mp4` records system launch → native static page → matching in-app loading page → loaded Home, with no uncovered black frame.
...
- The loading surface is one combined accessibility element labelled “Bookbinder. Opening the Atlas.”; its decorative mark is hidden. Ready and actionable failure transitions post one-shot VoiceOver announcements, while an immediately warm-ready scene stays quiet; the coordinator fixture asserts that behavior. There is no motion or progress animation, so Reduce Motion requires no alternate behavior.
