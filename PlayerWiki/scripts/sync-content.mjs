...
const playerWikiRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const repositoryRoot = path.resolve(playerWikiRoot, '..');
const sourceDataPath = path.join(repositoryRoot, 'GameWiki', 'generated', 'wiki-data.json');
...
const family = id => source.visualAssets.families.find(entry => entry.id === id);
const runtimeAsset = (familyID, semanticKey) => family(familyID)?.assets.find(asset => asset.role === 'runtime' && asset.semanticKey === semanticKey);

const safeFileName = value => `${String(value).replaceAll(/[^a-zA-Z0-9_-]/g, '-')}.png`;
...
for (const resource of source.resources) {
  const asset = runtimeAsset('resource-sprites-v1', `resources/profiles/inventory/${resource.id}`);
  resources.push({
    id: resource.id,
    slug: resource.slug,
    name: resource.name,
...
for (const item of source.items) {
  const asset = runtimeAsset('exploration-loose-items-v1', `catalogue-item/${item.id}`)
    ?? runtimeAsset('exploration-catalogue-objects-v1', `catalogue-item/${item.id}/identified`);
  items.push({
    id: item.id,
    slug: item.slug,
    name: item.name,
...
const stations = source.stations
  .filter(station => station.disposition === 'settled' && station.lifecycle !== 'removedCompatibility')
  .map(station => ({
    id: station.id,
    slug: station.slug,
    name: station.name,
...
const terrain = [];
for (const asset of family('terrain-production-pack-v1')?.assets.filter(asset => asset.role === 'runtime' && asset.semanticKey.includes('/macro/')) ?? []) {
  const name = asset.semanticKey.match(/macro\/(.+?)-semantic/)?.[1] ?? asset.semanticKey;
  terrain.push({ name, assetURL: await publishAsset(asset, 'terrain', safeFileName(name)) });
}

const writingAsset = family('writing-parchment-v1')?.assets.find(asset => asset.semanticKey === 'runtime/writing.parchment.handmade-v1');
const writingAssetURL = await publishAsset(writingAsset, 'writing', 'writing-parchment.png');
...
await writeFile(outputDataPath, `${JSON.stringify(playerContent, null, 2)}\n`, 'utf8');
console.log(`Synced ${resources.length} resources, ${items.length} items, ${travellers.length} live people and ${stations.length} current places.`);
