...
export default async function ResearchNodeDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SiteFrame sidebar>
    ...
    <RelatedGuides links={[{ label: 'Research directory', href: '/research' }, { label: 'Research guide', href: '/systems/research' }, { label: 'Library collections', href: '/services/library' }, ...(station ? [{ label: station.name, href: `/places/${station.slug}` }] : [{ label: 'Workshop and Research', href: '/places/workshop' }]), { label: 'All resources', href: '/resources' }]} />
  </SiteFrame>;
}
