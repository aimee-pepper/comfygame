...
export default async function EquipmentDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SiteFrame sidebar><GuideBreadcrumbs items={[{ label: 'Reference', href: '/equipment' }, { label: 'Equipment', href: '/equipment' }, { label: item.name }]} /><div className="entity-heading"><PixelImage src={item.assetURL} alt={`${item.name} icon`} size={96} /><PageIntro eyebrow={`${humanize(item.rarity)} ${humanize(item.gear.slot)}`} title={item.name} summary={item.summary} /></div>
    ...
    <section className="article-section two-column"><div><h2>Storage and swapping</h2><p>Stored and Waiting pieces can be equipped at Home. A piece worn by another person is still shown and can be used in a same-slot swap. Gear carried in the active world remains visible but cannot be changed until the party returns.</p></div><div><h2>Trading and recycling</h2><p>{item.tradingPostDisposition === 'sellable' ? 'This item can be sold at the Trading Post.' : 'This item is protected from ordinary Trading Post sale.'} {item.recyclerDisposition === 'recyclable' ? 'The Recycler can preview and dismantle this gear.' : 'This item is not accepted as recyclable gear.'}</p></div></section>
    <RelatedGuides links={[{ label: 'All equipment', href: '/equipment' }, ...(item.tradingPostDisposition === 'sellable' ? [{ label: 'Trading offer and sale terms', href: '/trading' }] : []), ...(item.recyclerDisposition === 'recyclable' ? [{ label: 'Recycler return reference', href: '/recycling' }] : []), { label: 'Combat guide', href: '/systems/combat' }, { label: 'Conditions and effects', href: '/statuses' }, { label: 'Techniques and Gambits', href: '/techniques' }, { label: 'Party, Gear and Gambits', href: '/systems/party-preparation' }, { label: 'Blacksmith construction', href: '/crafting/blacksmith' }, { label: 'Armoury rebuilding', href: '/crafting/armoury' }]} />
  </SiteFrame>;
}
