...
export default function RecyclingDirectory() {
  ...
  return <SiteFrame sidebar>
    ...
    <section className="article-section note-card">
      <h2>Choose the current physical piece</h2>
      <p>The Recycler is available after its 15-Essence foundation is complete. A recovery preview belongs to one current Stored or Waiting physical piece and its current state, so it has no durable player-facing transaction ID. This directory records the authored standard profiles; use Noll’s current preview for the exact selected holding.</p>
    ...
    <section className="article-section note-card"><h2>Confirm only the displayed preview</h2><p>Opening a preview, changing its selected receipt samples, Cancel, or Back does not consume anything. Only a completed Dismantle removes the exact piece and adds the displayed resources or returned samples. If the selected piece or current stores changed, close the preview and select it again.</p></section>
