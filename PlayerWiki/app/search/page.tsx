  ...
  const categories = [
    ...
    { label: 'People and records', entries: content.cast.flatMap((entry) => [
      ...
      ...entry.diaryPages.map((page) => ({
        ...
        type: page.worldHint ? 'Spoiler-marked location clue' : 'Book page',
        assetURL: null,
        alt: '',
      })),
    ]) },
    { label: 'Village', entries: villageBuildings.map(entry => { const service = serviceGuides.find((candidate) => candidate.stationID === entry.id); return { name: entry.name, summary: entry.status === 'scheduled' ? `Planned — ${entry.blurb} This place is not available yet.` : `${entry.blurb}${service ? ` ${service.summary}` : ''}`, href: `/buildings/${entry.slug}`, type: entry.status === 'scheduled' ? 'Planned destination' : 'Available Village destination', assetURL: entry.assetURL ?? entry.contextAssetURL, alt: entry.assetURL ? `${entry.name} building visual` : `${entry.zone} town setting` }; }) },
    { label: 'Economy references', entries: [
      { name: 'Trading Post offers', summary: 'Changing shop stock, purchase terms, sale prices, where purchases go, and why a trade may be unavailable.', href: '/trading', type: 'Current economy reference', assetURL: content.stations.find((station) => station.id === 'trading_post')?.assetURL ?? null, alt: 'Trading Post building visual' },
      { name: 'Recycler returns', summary: 'Standard salvage patterns, recovery from recorded construction materials, protected pieces, and confirmation before dismantling.', href: '/recycling', type: 'Current economy reference', assetURL: content.stations.find((station) => station.id === 'recycler')?.assetURL ?? null, alt: 'Recycler building visual' },
    ] },
