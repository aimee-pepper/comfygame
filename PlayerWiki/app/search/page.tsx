...
export default function SearchPage() {
  ...
  const categories = [
    ...
    { label: 'Village', entries: villageBuildings.map(entry => { const service = serviceGuides.find((candidate) => candidate.stationID === entry.id); return { name: entry.name, summary: entry.status === 'scheduled' ? `${entry.blurb} Scheduled; not yet a live player route.` : `${entry.blurb}${service ? ` ${service.summary}` : ''}`, href: `/buildings/${entry.slug}`, type: entry.status === 'scheduled' ? 'Scheduled destination' : 'Current Village destination', assetURL: entry.assetURL ?? entry.contextAssetURL, alt: entry.assetURL ? `${entry.name} building visual` : `${entry.zone} town setting` }; }) },
