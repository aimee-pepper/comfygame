...
export default async function PlaceDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SiteFrame sidebar><PageIntro eyebrow={place.zone} title={place.name} summary={place.blurb} />
    <section className="article-section place-visuals" aria-label={`${place.name} town visual`}>
      {place.assetURL && <figure className="place-visual place-building"><img src={place.assetURL} alt={`${place.name} building visual`} /><figcaption>The current town building visual for {place.name}.</figcaption></figure>}
      <figure className="place-visual place-context"><img src={place.contextAssetURL} alt={`${place.zone} town context`} /><figcaption>{place.assetURL ? `The current ${place.zone} context around this service.` : `The current retained town context for ${place.name}.`}</figcaption></figure>
    </section>
