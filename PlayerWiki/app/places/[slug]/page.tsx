...
async function PreservedFormerPlaceDetail({
  ...
  const publishedRecipes = place.id === 'survey_post' ? [] : recipes;
  const actions = [
  ...
  return (
    <SiteFrame sidebar>
      ...
      {place.id === 'blacksmith' && <section className="article-section"><h2>Build it with Halloway</h2><p>Recruiting <Link href="/people/halloway">Halloway</Link> reveals this foundation in Home → Make. Its complete current requirement is <strong>{blacksmithFirstUse.construction}</strong>.</p><div className="definition-grid"><div><h3>Completion teaches, not grants</h3><p>The completed T1 Forge offers Pointed Blade, Cutting Blade, Hand Maul and Shield. It does not create gear or grant free material stock.</p></div><div><h3>Review again after a refusal</h3><p>A shortfall, changed requirements, unavailable Halloway, or failed save does not spend stock or teach the Schematic. Review the displayed foundation again before trying.</p></div></div><p><Link href="/services/blacksmith">Follow the first-use journey</Link> · <Link href="/crafting/blacksmith">Open Pointed Blade construction</Link></p></section>}
      {place.id === 'survey_post' && <section className="article-section"><h2>Build it with Mara</h2><p>Recruiting <Link href="/people/mara">Mara</Link> reveals this foundation in Home → Study. Its complete current requirement is <strong>{surveyPostFirstUse.construction}</strong>.</p><div className="definition-grid"><div><h3>Completion opens research, not a tool bin</h3><p>The first durable construction opens Field Instruments research. It does not create an instrument, Storehouse stack, supply, observation, or field reading.</p></div><div><h3>Review again after a refusal</h3><p>A shortfall, changed requirements, unavailable Mara, or failed save does not spend stock or grant a capability. Review the displayed foundation again before trying.</p></div></div><p><Link href="/services/survey-post">Follow Mara’s first-reading journey</Link> · <Link href="/crafting/instruments">Open Field Instruments</Link></p></section>}
      ...
      <section className="article-section">
        ...
        {publishedRecipes.length ? <div className="table-wrap"><table><thead><tr><th>Recipe</th><th>Output</th><th>Exact inputs</th><th>Ready when</th></tr></thead><tbody>{publishedRecipes.map((recipe) => { const item = content.items.find((entry) => entry.name === recipe.result); const outputHref = item ? (item.gear ? `/equipment/${item.slug}` : `/items/${item.slug}`) : null; return <tr key={recipe.id}><td><Link href={`/crafting/${recipe.system}`}>{recipe.name}</Link></td><td>{outputHref ? <Link href={outputHref}>{recipe.result}</Link> : recipe.result}</td><td>{recipeRequirements(recipe)}</td><td>{recipeReadiness(recipe)}</td></tr>; })}</tbody></table></div> : place.id === 'survey_post' ? <p>Field Instruments are permanent Research capabilities. Paid precision improvement stays unpublished until its exact typed quote and receipt are live.</p> : <p>No current crafting recipe is published for this place.</p>}
      </section>
