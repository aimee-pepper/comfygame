...
function PreservedFormerKnowledgeRecordsGuide() {
  ...
  return <SiteFrame sidebar>
    ...
    <section className="article-section journey-strip">{library?.assetURL && <Link href="/services/library"><PixelImage src={library.assetURL} alt="Library building visual" size={64} /><span><strong>Library</strong><small>Open current Diaries, People, Dictionary, Notes, and History collections.</small></span></Link>}{traveller?.assetURL && <Link href={`/people/${traveller.slug}`}><PixelImage src={traveller.assetURL} alt={`${traveller.name} character visual`} size={64} /><span><strong>People and records</strong><small>Read each person’s complete authored record, with location-hint stages clearly marked.</small></span></Link>}</section>
    <section className="article-section"><h2>Current Library collections</h2><div className="definition-grid"><div><h3>Diaries</h3><p>Read recovered diary pages exactly as they are recorded. Attached teaching is shown with that page when current content provides it.</p></div><div><h3>People</h3><p>Each Player Wiki person page keeps the complete current authored book record together. Its location-hint stages are clearly spoiler-marked rather than hidden behind unrelated sections.</p></div><div><h3>Dictionary</h3><p>Review known Sigils and Compounds. Unknown meanings remain unknown until they are learned.</p></div><div><h3>Notes and History</h3><p>Open recovered notes and visited-world history from their current collection; older entries may remain represented even where their original prose is unavailable.</p></div></div><p><Link href="/services/library">Open Library collections</Link> · <Link href="/people">Browse complete people records</Link> · <Link href="/glossary">Open the glossary</Link></p></section>
