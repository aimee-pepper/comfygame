...
import { economySafetyRules, materialPriceRules } from '@/lib/crafting-overview';
import {
...
export default function TradingDirectory() {
  ...
  return <SiteFrame sidebar>
    ...
    <section className="article-section"><h2>Material offers now and in the intended system</h2><TruthPair current="The Trading Post currently mixes counted resources with individual material samples. The shelf and confirmation show the exact stock, quantity, and price available now." accepted="Mined resources and ordinary flora will be bought and sold by exact material or subtype and quantity, with no quality variants. Creature materials will be bought and sold by subtype, quality, and quantity, with species and source history available inside the stack. The Trading Post will never substitute a different material or quality for the one you chose." /></section>
    <section className="article-section"><h2>Planned raw-material prices</h2><p>The intended price depends on the physical material and, for creature materials, its quality—not its species name, colour, source world, or a hidden recipe score. These prices are planned and do not replace the current shelf values until the material-overhaul slice ships.</p><div className="table-wrap"><table><thead><tr><th>Material class</th><th>You sell for</th><th>You buy for</th></tr></thead><tbody>{materialPriceRules.map(([material, sell, buy]) => <tr key={material}><td><strong>{material}</strong></td><td>{sell}</td><td>{buy}</td></tr>)}</tbody></table></div><p>Poor, Common, Rare, and Exceptional creature materials use ×0.5, ×1, ×2, and ×4 value, rounded up to at least 1 Gold. A special merchant request can offer a different visible quote; an ordinary transaction cannot.</p></section>
    <section className="article-section"><h2>Planned economy safeguards</h2><div className="definition-grid">{economySafetyRules.map(([rule, behavior]) => <div key={rule}><h3>{rule}</h3><p>{behavior}</p></div>)}</div></section>
