...
export default async function StatusDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SiteFrame sidebar>
    ...
    <section className="article-section two-column"><div><h2>Clear or prevent</h2><p>{status.clearing}</p>{items.length ? <ul className="compact-list">{items.map((item) => <li key={item.id}><Link href={`/items/${item.slug}`}>{item.name}</Link> — {item.summary}</li>)}</ul> : null}</div><div><h2>Where the effect applies</h2><p>{status.persistence}</p><p>Read the condition details and the combat or Field Kit preview before choosing an action. If the target or situation changes, the condition remains until a valid action clears it.</p></div></section>
