...
async function PreservedFormerServiceDetail({
  ...
  return (
    <SiteFrame sidebar>
      ...
      {guide.slug === 'blacksmith' && <section className="article-section"><h2>Halloway and the first Forge recipes</h2><p>After <Link href="/people/halloway">Halloway</Link> joins the Village, her foundation appears in Home → Make. Her current request is: <em>“{blacksmithFirstUse.correctedRequest}”</em></p><ol className="numbered-guide">{blacksmithFirstUse.journey.map((step) => <li key={step}>{step}</li>)}</ol><p><Link href="/buildings/blacksmith">Review the Blacksmith foundation</Link> · <Link href="/crafting/blacksmith">Review Pointed Blade construction</Link> · <Link href="/systems/equipment-materials">Read equipment and material effects</Link></p></section>}
      {guide.slug === 'blacksmith' && <section className="article-section two-column"><div><h2>Pointed Blade stays specific</h2><ul className="compact-list">{blacksmithFirstUse.pointedBlade.map((line) => <li key={line}>{line}</li>)}</ul><p>{blacksmithFirstUse.stockBoundary}</p></div><div><h2>Refit one exact piece</h2><ul className="compact-list">{blacksmithFirstUse.reforgeBoundary.map((line) => <li key={line}>{line}</li>)}</ul></div></section>}
