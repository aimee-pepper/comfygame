...
import { worldGenerationPlan, writingGuaranteeRules } from '@/lib/crafting-overview';
...
export default function WorldReferencePage() {
  ...
  return <SiteFrame sidebar><PageIntro eyebrow="Field reference" title="World conditions, terrain, and Flora" summary="Use this reference to understand the current world facts you can inspect without predicting a bound map’s hidden tiles, deposits, sites, creatures, or exact Flora." />
    ...
    <section className="article-section note-card"><h2>Worlds are combinations, not biomes</h2><p>A bound world is shaped by eight conditions at once. In the current game, those pressures influence the result without guaranteeing an exact ground or resource. The intended direct-writing grammar below gives different promises to ground, liquid, base resources, and ecological materials. In both systems, the bound world keeps its generated answer when you revisit it instead of rolling a replacement map.</p><nav aria-label="World guide links"><Link href="/systems/world-writing">World Writing</Link><Link href="/systems/exploration">Exploration</Link><Link href="/references/resource-world-numbers-decided-so-far">Accepted world tuning</Link><Link href="/buildings/survey-post">Survey Post</Link></nav></section>
    <section className="article-section"><h2>Intended written guarantees</h2><div className="table-wrap"><table><thead><tr><th>What you write</th><th>What the finished system promises</th></tr></thead><tbody>{writingGuaranteeRules.map(([request, promise]) => <tr key={request}><td><strong>{request}</strong></td><td>{promise}</td></tr>)}</tbody></table></div><p>The guaranteed source uses the published first-pass cluster yield. Exact positions inside its distance bands, additional deposits, hazards, and unwritten world facts remain generated. Every mandatory route and written guarantee stays within the entry-connected traversal graph.</p></section>
