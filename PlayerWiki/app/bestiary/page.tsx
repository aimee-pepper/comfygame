  ...
  return (
    ...
    <CreatureHabitatDecisions />
      ...
      <section className="article-section">
        ...
        <TruthPair current="Build376 enables solid creature materials in newly written books: Fur Pelt, Overlapping Scales, Armoured Scales, Chitin, Chitin Plate, Shell, Protective Spines, Flight Feathers, Contour Feathers and useful Horn. Actual body parts determine what can be recovered. Existing books, creatures and earned stock keep their saved rules; Hide and Bone retain their existing recovery paths. The source measurements used for quality remain distinct from the older property calculations below." accepted="A material’s family, type, and subtype decide which recipe slot accepts it. Its inherited numerical properties then contribute to concrete item statistics, while its four-band quality controls the strength of those contributions." acceptedLabel="Intended design" />
        <p>The following property table documents older retained material paths. It does not define the new solid-part quality formula or imply that new fluid sources are delivered.</p><div className="table-wrap"><table><thead><tr><th>Material property</th><th>Legacy creature-derived calculation</th></tr></thead><tbody>{creatureMaterialPropertyDerivations.map(([property, calculation]) => <tr key={property}><td><strong>{property}</strong></td><td>{calculation}</td></tr>)}</tbody></table></div>
      ...
      <section className="article-section note-card">
        <h2>Current first Leather path</h2>
        ...
        <p>Corrin can turn matching parts and Salt into Leather for a Leather Guard. The earlier woven garments need no animal parts or Salt. The one-part Leather recipe and garments are delivered in323. Each Leather uses one eligible Skin/Hide and one Salt. Exact source portions remain preserved inside the agreed subtype-and-quality stack; build345 corrects the property-based visible split in Return and Storehouse.</p>
