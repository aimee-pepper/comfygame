...
export default function LootAndMaterialsPage() {
  return <SiteFrame sidebar>
    ...
    <section className="article-section">
      ...
      <TruthPair current="World resources are mostly counted reserves. Creature and other physical materials can still be separate source-bearing samples. Return, trading, crafting, and recycling therefore do not all present stock in the same way." accepted="A creature or harvest produces a recognizable material type and subtype. Species-specific items of the same subtype and quality share a default stack; expanding it reveals species, source world, inherited colour, quantity, and contribution values." acceptedLabel="Intended design" />
    </section>
    <section className="article-section">
    ...
    <section className="article-section">
      ...
      <div className="definition-grid"><div><h3>Resource quality</h3><p>{qualityBands.join(' · ')}</p><p>Subtype + quality is the default stack. When quality affects the result, the player chooses the exact stack.</p></div><div><h3>Example stack</h3><p><strong>{canonicalStackExample}</strong></p><p>Each species-specific item and inherited colour remains inspectable inside it.</p></div><div><h3>Other views</h3><p>Sort or group by material, quality, species, source, colour, quantity, or recency without changing what the player owns.</p></div></div>
    </section>
    ...
    <RelatedGuides links={[{ label: 'Resources', href: '/resources' }, { label: 'Crafting systems', href: '/crafting' }, { label: 'Expedition Return', href: '/journey' }, { label: 'Trading offers', href: '/trading' }, { label: 'Recycler returns', href: '/recycling' }, { label: "What's changing", href: '/guide-status' }]} />
  </SiteFrame>;
}
