...
export default function CraftingSystemsPage() {
  return <SiteFrame sidebar>
    ...
    <section className="article-section"><h2>How recipes work now and how they will change</h2><TruthPair current="Crafting currently combines counted resources, individual gathered materials, fixed costs, and hidden Hardness, Density, Insulation, Flexibility, Lustre, or Reactivity requirements. Each station page explains the rules its current recipes use." accepted="Recipes will combine fixed ingredients with clear material categories. Mined ingredients use their exact plain name and quantity, with no quality choice. For approved biological ingredients, you will choose a physical type or subtype and a Poor, Common, Rare, or Exceptional quality; the preview will show how that choice changes the finished item’s real statistics." acceptedLabel="Intended design" /></section>
    <section className="article-section"><h2>How recipes name ingredients</h2><p>A recipe can combine fixed ingredients with one or more material choices. A fixed ingredient names one exact resource, such as Sand or Gold. A category ingredient accepts any owned material from its displayed broad category, type, or subtype. A recipe narrows a choice only when the physical job genuinely requires it.</p><div className="table-wrap"><table><thead><tr><th>Ingredient scope</th><th>Example</th><th>Recipe meaning</th></tr></thead><tbody>{materialIdentityHierarchy.map(([level, example, use]) => <tr key={level}><td><strong>{level}</strong></td><td>{example}</td><td>{use}</td></tr>)}</tbody></table></div><ul className="compact-list"><li>The picker shows only compatible owned stacks.</li><li>Mined materials require only exact material and quantity selection.</li><li>The player chooses the exact subtype, quality, and quantity when a quality-bearing biological ingredient offers more than one valid option.</li><li>A recipe never silently spends a higher quality than the one confirmed.</li><li>Species history can remain visible without turning every species into a separate recipe ingredient.</li></ul></section>
