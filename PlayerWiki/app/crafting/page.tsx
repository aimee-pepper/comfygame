...
import { craftedAppearanceRules, craftedQualityRules, materialIdentityHierarchy, materialPropertyProblems, materialScoreBoundary, processingConversions, processingQualityRules, qualityRules, sourceLotRules } from '@/lib/crafting-overview';
...
export default function CraftingSystemsPage() {
  return <SiteFrame sidebar>
    ...
    <section className="article-section"><h2>Optional gem inlay — planned</h2><TruthPair current="Gem inlay is not yet available; Quartz keeps its existing functional recipes." accepted="Eligible Blacksmith gear with a real metal fitting will offer an optional one-gem inlay during crafting. Quartz, Garnet or Beryl supplies the small decorative stone and its retained source colour. Normal power, quality and unlocks stay the same, and no gem is required to make the item. Later suitable specialist fittings follow their coordinated crafting updates. Refitting preserves the inlay; ordinary recycling returns its original gem once. Separate removal or replacement services are outside this initial crafting feature." acceptedLabel="Decided intended" /><p><Link href="/resources">Read the planned secondary-gem sources</Link>. These drops will arrive together with their first usable fitting and inventory icons.</p></section>
    ...
    <section className="article-section"><h2>Finished workmanship is separate from material quality</h2><p>Identity-bearing primary sockets share 70% of a finished item’s ordinary quality score and structural secondary sockets share 30%. Poor, Common, Rare, and Exceptional creature materials contribute ranks 0–3. An ungraded designated socket contributes rank 1, so a normal recipe made entirely from ungraded stock is Fine unless that exact recipe has a fixed named result. Add the weighted ranks and round half up once.</p><div className="table-wrap"><table><thead><tr><th>Rounded score</th><th>Finished name</th><th>Meaning</th></tr></thead><tbody>{craftedQualityRules.map(([score, name, meaning]) => <tr key={score}><td>{score}</td><td><strong>{name}</strong></td><td>{meaning}</td></tr>)}</tbody></table></div><p>Accepted intended direction, not implemented: refine an existing eligible piece toward Peerless. One Mote plus a maximum-level shop plus its attending matching keeper would guarantee success; one or two advantages would offer a disclosed chance. Partial odds, spending on a miss, previews, and old bad-luck progress remain open. The old 3%/5% chance and twentieth-copy guarantee are reopened and must not be applied alongside this accepted direction.</p></section>
    ...
    <section className="article-section"><h2>How material colours appear on finished items</h2><div className="definition-grid">{craftedAppearanceRules.map(([item, behavior]) => <div key={item}><h3>{item}</h3><p>{behavior}</p></div>)}</div></section>
