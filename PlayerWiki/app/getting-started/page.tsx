...
export default function GettingStarted() {
  ...
  return <SiteFrame sidebar><PageIntro eyebrow="Start here" title="Getting started" summary="Bookbinder alternates between preparing at home, writing a world, exploring it, and bringing discoveries back to strengthen the next expedition." />
    ...
    <section className="article-section"><h2>The first Writing lesson</h2><TruthPair current={starterRuneFlow.current} accepted={starterRuneFlow.intended} acceptedLabel="Decided opening" /><p><strong>If the introduction is interrupted:</strong> {starterRuneFlow.recovery}</p><p><strong>Existing campaigns:</strong> {starterRuneFlow.legacy}</p></section>
    <section className="article-section two-column"><div><h2>Good first priorities</h2><p>Settings → Tutorials lets you stop automatic teaching without hiding ordinary opening instructions or necessary choices. Field Notes remains readable, and Replay on its screen requests one lesson in its valid context without turning automatic tutorials back on. Got it now remembers your acknowledgement across expeditions and reopening without claiming you performed the taught action. Not now postpones an unfinished lesson; manual replay remains available.</p><ul><li>Read the current Page before changing it.</li><li>Carry at least one healing supply when possible.</li><li>Inspect unfamiliar terrain and flora before entering.</li><li>Keep track of the entry portal and revealed exits.</li><li>Return before instability or party health becomes unmanageable.</li></ul></div><div className="note-card"><h3>Your campaign keeps what matters</h3><p>Your campaign book saves the Cottage, party, knowledge, and everything successfully brought home. Each generated world may be temporary, but those lasting discoveries are not.</p></div></section>
