...
export default function GuideStatusPage() {
  ...
  return <SiteFrame sidebar>
    ...
    <section className="article-section"><h2>Crafting and processing</h2><p>Open a system for its complete current recipe table and planned design. Choices that still need Aimee’s decision stay clearly labeled instead of being presented as final.</p><div className="status-card-grid">{craftingFamilyStatus.map((entry) => <article className="status-card" key={entry.slug}><p className="status-pill">{entry.status}</p><h3><Link href={statusHref(entry.slug)}>{entry.name}</Link></h3><TruthPair current={entry.current} accepted={entry.accepted} /></article>)}</div></section>
