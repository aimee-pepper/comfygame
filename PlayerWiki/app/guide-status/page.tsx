...
import { acceptedChoices, correctionStatus, craftingFamilyStatus, laterDesignWork, openDecisions } from '@/lib/player-guide-status';
...
export default function GuideStatusPage() {
  ...
  return <SiteFrame sidebar>
    ...
    <section className="article-section note-card"><h2>Choices for Aimee</h2><p>Everything not listed here is owned by the design and implementation passes above.</p>{openDecisions.map((entry) => <div key={entry.title}><h3>{entry.title}</h3><p>{entry.body}</p></div>)}</section>
  </SiteFrame>;
}
