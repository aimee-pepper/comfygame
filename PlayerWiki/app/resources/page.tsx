...
export default function ResourcesPage() {
  return (
    ...
    <FieldInteractionDecisions />
    <section className="article-section"><h2>Rare secondary gems — planned</h2><TruthPair current="Quartz has its existing uses. The wider secondary-gem system and decorative inlay are not yet delivered." accepted="Suitable stone and metal deposits can occasionally give one extra useful gem when fully worked out. First-pass plans use Quartz, Garnet and Beryl with overlapping host pools and a four-to-eight-percent chance based on actual richness. The host’s usual tool and work apply. Gems are optional crafting decoration, never required opening ingredients; each source can pay once, and existing saved worlds retain their rewards." acceptedLabel="Decided intended" /><p>New gems will arrive with an optional decorative fitting on eligible Blacksmith gear. Each gem remains one ungraded exact-name stack, with selected colour retained inside it. <Link href="/references/design-decisions-september-4">Read the gem and crafting plan</Link>.</p></section>
