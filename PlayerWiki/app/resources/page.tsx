    ...
    <SiteFrame sidebar>
      ...
      <section className="article-section">
        ...
        <TruthPair
          ...
          accepted="Mined stock will use one exact-name quantity stack with no quality: Sand is Sand and Gold is Gold, always in the normal/green presentation. Approved biological stock will use recognizable categories, types, subtypes, and four quality bands, while species, world, colour, and inherited values remain available in expanded history."
      ...
      <section className="article-section">
        ...
        <div className="definition-grid"><div><h3>Mined resources</h3><p><strong>Sand · Gold · Granite</strong></p><p>One exact-name quantity stack each, with no quality variants. All use the normal/green presentation.</p></div><div><h3>Biological material quality</h3><p>{qualityBands.join(' · ')}</p><p>White, green, blue, and purple respectively. Each approved biological subtype and quality has its own default stack.</p></div><div><h3>How a biological stack reads</h3><p><strong>{canonicalStackExample}</strong>. Expand it to inspect every species-specific item, source world, inherited colour, quantity, and visible stat contribution.</p></div><div><h3>Inventory views</h3><p>The default shows mined materials by exact name and biological materials by subtype + quality. Alternate views never move or merge stock.</p></div><div><h3>Selection</h3><p>Mined recipes ask only for material and quantity. When biological quality affects a result, the player chooses which quality to spend.</p></div><div><h3>Peerless</h3><p>Peerless is reserved for legendary equipment, not raw materials.</p></div></div>
      </section>
