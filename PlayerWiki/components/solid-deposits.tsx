...
export function SolidDeposits() {
  return <section className="article-section note-card">
    ...
    <p><strong>Decided intended behavior:</strong> Substantial deposits and boulders block their physical base. With a qualifying Pick already available, deliberately step toward an adjacent harvestable deposit to mine it; the action should choose the applicable Pick automatically. A successful hit takes one world turn and keeps you beside it. The final hit clears its base; your next step enters the cleared square normally.</p>
    <p>If no available tool meets the requirement, the action spends no mining turn and takes no material. An unrelated selected tool must not block a qualifying available Pick. Automatic routes go around deposits or stop; they never initiate mining. The automatic-tool correction is decided and not yet delivered. Small loose stones, herbs and low gathering patches stay walkable, with their existing gathering or pickup actions.</p>
