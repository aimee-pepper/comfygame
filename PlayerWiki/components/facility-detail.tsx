...
export function FacilityDetail({ slug }: { slug: string }) {
  ...
  return <SiteFrame sidebar>
    ...
    {building.status === 'scheduled' ? <section className="article-section note-card">
      ...
      {building.id === 'blacksmith' && <><section className="article-section"><h2>Halloway and the first Forge recipes</h2><p>After <Link href="/people/halloway">Halloway</Link> joins the Village, her foundation appears in Home → Make. Her current request is: <em>“{blacksmithFirstUse.correctedRequest}”</em></p><ol className="numbered-guide">{blacksmithFirstUse.journey.map((step) => <li key={step}>{step}</li>)}</ol></section><section className="article-section two-column"><div><h2>Pointed Blade stays specific</h2><ul className="compact-list">{blacksmithFirstUse.pointedBlade.map((line) => <li key={line}>{line}</li>)}</ul><p>{blacksmithFirstUse.stockBoundary}</p></div><div><h2>Refit one selected piece</h2><ul className="compact-list">{blacksmithFirstUse.reforgeBoundary.map((line) => <li key={line}>{line}</li>)}</ul></div></section><section className="article-section note-card"><h2>Storage and relaunch</h2><ul className="compact-list">{blacksmithFirstUse.refusalAndRelaunch.map((line) => <li key={line}>{line}</li>)}</ul></section></>}
