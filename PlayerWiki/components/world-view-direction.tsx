...
export function WorldViewDirection() {
  return <section className="article-section note-card">
    ...
    <p><strong>Approved ordinary 3D trial — not yet delivered:</strong> Settings → Owner Tools → 3D Trials will offer one normally generated expedition with its own saved test state, using the current starting setup and normal costs. North stays up and east stays right in the square-projected three-quarter view. Ordinary 2D stays available; placeholders are allowed. Existing movement, tools, encounters, visibility, memory and fading remain authoritative.</p>
    <p>The small trial covers what is naturally encountered, followed by leaving normally and reopening the trial once as a quick save check. It is not a campaign test or full 3D readiness claim. Wider creature proposals and the unfinished zero-rune introduction are not prerequisites.</p>
