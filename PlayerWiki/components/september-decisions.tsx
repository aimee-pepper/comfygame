...
const updates: Record<DecisionTopic, { current?: string; decided: string; tuning?: string; open?: string }> = {
  ...
  progression: {
    ...
    open: 'An arranged development journey reaches Nessa, the Apothecary, a Salve and use after the next Bind. Two natural trips retained all healing materials. Nessa failed her first appearance chance, then the second world was too dark for her habitat; that trip selected Halloway, who also failed his own chance. Manual refinement now leaves 36 spendable Essence. The learned daylight clue led to a two-mark Moderate Sun page quoted at 14, unpurchased at this receipt. One journey with that page is the next bounded test; it does not guarantee Nessa. Full recruitment and healing-route affordability remain unproven. Home recovery is free. Phone delivery remains pending. T2 smelting and the first cloth/Leather specialist paths are specified; the wider campaign redesign remains separate work. Current directory order remains current behavior.',
  },
  exploration: {
    ...
    open: 'The full development test found connected growing habitat in 296 of 1,000 unwritten worlds, above the 25% review target. Actual Softwood appeared in 293 worlds; habitat and placed sources are separate measures. The original test failed on six Softwood or Coal shortfalls. Three are confirmed shared-space limits; the other three now pass focused runtime checks after protecting scarce required sites before flexible and optional choices. The original full-sample counts have not been remeasured. The single map-generation failure now passes focused checks after correcting how small flowing-water totals are divided into channels, with its exact water total preserved. All placed sources retained reachable working positions. Explicit written early-resource guarantees, natural pacing and phone delivery remain pending. Drainage is not part of this revision; no universal map-completion percentage is fixed.',
  },
  writing: {
    decided: 'The intended opening teaches Illumination and Sun through a safe introductory excursion, then visibly connects the player’s first written choice to the world they bind. Unwritten features remain generated. Deliberately writing Iron into Substrate promises one reachable two-deposit cluster within the ordinary source budget; repeated statements and strength, size or count words do not multiply that guarantee. The complete promise is checked before spending for Bind. Traveller clues use recognizable world facts and vocabulary the player has had a chance to learn; existing knowledge and Pages remain in older saves.',
    open: 'Early maker clues and the next water, River, ground, and Iron lessons are specified. The Iron-first guarantee is being implemented; natural delivery and pacing still need verification. This adds no new Coal, Clay or Resin writing vocabulary, and the wider campaign order remains separate.',
  },
  appearance: {
    ...
    open: 'Three-quarter top-down presentation, local water heights, clear trunk-base targeting, and foreground fade are decided. Actual 3D rendering with a fixed orthographic camera and sprites is under consideration, not selected. Exact artwork dimensions, composition, and fade timing await the bounded in-game proof. Final world-entry layer sizes, placement, and movement remain open. Aimee’s Library work is in progress; sky/cloud homework is optional study work.',
  },
  people: {
    decided: 'The intended priority is Vance, then Nessa and Halloway, then Bryn, Corrin, and Noll. Nessa is sought in fresh growing land, Halloway near workable Iron, and Corrin near fibre-bearing growth. Suitable ground counts together across the area connected to the entrance, while retaining each plant’s habitat requirements. Early makers no longer wait behind the old three-recruit requirement. Old clues keep their discovery value and receive updated hints.',
