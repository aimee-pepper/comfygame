...
export function FieldInteractionDecisions({ study = false }: { study?: boolean }) {
  return <section className="article-section note-card">
    ...
    <p><strong>Current in build 319:</strong> actual Apex creatures have functional labels on the existing visible field/minimap markers, current-sight details and encounter header. Ordinary stationary creatures do not. Visibility rules are unchanged; final styling remains in progress.</p>
    <p><strong>Also decided, pending implementation:</strong> world-coloured tree wood carried into Logs and equipment, and category controls for optional monster and gathering notices. Older wood stock keeps its lawful uses; muting notices keeps damage, danger and required decisions visible.</p>
