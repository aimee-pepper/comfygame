...
const legendEntries=[["#78b85e","vegetation"],["#f4f0e8","ordinary mob"],["#ff4f62","apex"],["#ee8d48","active flora"],[contentColors.resource,"resource"],[contentColors.portal,"portal"],[contentColors.site,"site"],[contentColors.diary,"writing"],[contentColors.traveller,"traveller"]];
...
function castCensus(rows){$("#generated-cast-census").innerHTML=rows.length?`<div class="census-table">${rows.map(row=>`<div class="census-row"><span class="name">${title(row.name)}</span><span class="detail">${row.placements?"placed":"not placed"} · cast ${row.id.slice(-6)}</span><strong>${row.placements}</strong></div>`).join("")}</div>`:`<p class="empty-census">No species generated.</p>`}
function threatCensus(world){const rows=[...world.apexes.map(row=>({...row,id:`Apex · ${row.id}`})),...world.hostileFlora.map(row=>({...row,id:`Active flora · ${row.id}`}))];census("#threat-census",rows)}
...
function renderWorld(index){
  ...
  $("#headline-metrics").innerHTML=metric(world.mobs.length?total(world.mobs):0,"ordinary mobs")+metric(floraTiles,"vegetated tiles")+metric(resourceQuantity,"obtainable resources")+metric(poi,"points of interest")+metric(world.diagnostics.initialTurnBudget,"initial turn budget")+metric(world.diagnostics.apexPlaced?"yes":"no","apex placed");
  census("#flora-census",world.flora);census("#mob-census",world.mobs);castCensus(world.generatedCast);threatCensus(world);census("#resource-census",world.resources,{resources:true});census("#poi-census",world.pointsOfInterest);census("#terrain-census",world.terrain);
  const writing=[...world.writings,...world.travellers.map(id=>({id:`Traveller · ${id}`,quantity:1}))];census("#writing-census",writing);
  $("#flora-total").textContent=`${floraTiles} tiles · ${world.diagnostics.floraSpeciesCount} species`;$("#mob-total").textContent=`${total(world.mobs)} placed · ${world.mobs.length} placed species`;$("#generated-cast-total").textContent=`Generated cast: ${world.generatedCast.length} species`;$("#resource-total").textContent=`${resourceQuantity} obtainable`;$("#poi-total").textContent=`${poi} placed`;$("#writing-total").textContent=`${total(writing)} records`;$("#terrain-total").textContent=`${world.width*world.height} tiles`;$("#census-grid").hidden=false;
  $("#batch-results").querySelectorAll("button").forEach((button,i)=>button.setAttribute("aria-current",String(i===index)));
}
function renderBatch(){const panel=$("#batch-panel");panel.hidden=state.worlds.length<2;$("#batch-results").innerHTML=state.worlds.map((world,index)=>`<button class="batch-card" data-index="${index}" aria-current="${index===state.current}"><strong>Seed ${world.seed}</strong><span>${total(world.mobs)} ordinary mobs · ${total(world.apexes)} apex · ${total(world.hostileFlora)} active flora</span><span>${total(world.flora)} vegetation · ${world.resources.reduce((sum,row)=>sum+row.quantity,0)} resources · ${total(world.pointsOfInterest)} POIs</span></button>`).join("");$("#batch-results").querySelectorAll("button").forEach(button=>button.addEventListener("click",()=>renderWorld(Number(button.dataset.index))))}
