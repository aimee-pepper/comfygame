...
const pngFiles=[];const collectPNGs=folder=>{for(const name of fs.readdirSync(folder)){const file=path.join(folder,name),stat=fs.statSync(file);if(stat.isDirectory())collectPNGs(file);else if(name.endsWith(".png"))pngFiles.push(file)}};collectPNGs(artifact);pngFiles.sort();const pngBundleReceipt=pngFiles.map(file=>`${path.relative(artifact,file)}:${crypto.createHash("sha256").update(fs.readFileSync(file)).digest("hex")}`).join("\n");assert.equal(pngFiles.length,62);assert.equal(crypto.createHash("sha256").update(pngBundleReceipt).digest("hex"),"f6ba0ff78911cd7ddcecdc3696053be8f6bf6414be7481788320260ff3bb9c40","accepted production/evidence PNG bytes may change only for the reconciled Rainwashed Shore copy");
const productionPNGs=pngFiles.filter(file=>file.includes(`${path.sep}scenes${path.sep}`)||file.includes(`${path.sep}parts${path.sep}`)||file.includes("first-map-crop")),productionReceipt=productionPNGs.map(file=>`${path.relative(artifact,file)}:${crypto.createHash("sha256").update(fs.readFileSync(file)).digest("hex")}`).join("\n");assert.equal(productionPNGs.length,38);assert.equal(crypto.createHash("sha256").update(productionReceipt).digest("hex"),"4eb1076eb8d4640a5467d3343167f55eed3b1c74ca2cefec5727cb5b5bda9184","all production scene, part and first-map continuity raster bytes must remain frozen");
...
const rainwashedDescription="Stone shelves break a wide run of shallow and deep water. Your Archipelago mark divided the route, while only the ground nearest the entry remained clearly visible.";
assert.equal(fixtures.starter_rainwashed_shore.description,rainwashedDescription);assert.equal(fixtures.visible_site_candidate.description,rainwashedDescription);
...
const descriptionOnly=structuredClone(fixtures.starter_rainwashed_shore),rainwashedCommands=arrivalSceneCommands(descriptionOnly);descriptionOnly.description="Stone shelves break a wide run of shallow and deep water. Receipt copy does not seed scene commands.";assert.deepEqual(arrivalSceneCommands(descriptionOnly),rainwashedCommands,"rules-owned description copy has no scene authority");
